...
\begin{align}
\zik = \xk - \frac{m^{(k)}_{c_i}\left(\xk\right)}{\left\|\gmcik\right\|^2} \gmcik. \label{define_z}
\end{align}
...
What isn't clear?
\end{comment}
We are interested in constructing a cone of directions known to be feasible: namely those unit vectors that make a negative dot product with each nearly active constraint.
We measure the distance between the direction $\huk$ and the nearest direction that is not feasible by computing
\begin{align}
...
Within the following assumption, remember that $\zik$ is defined by \cref{define_z}.
\begin{assumption}
...
\label{bounded_condition_numbers}
Let $\activeconstraintsk$ be defined by \cref{define_activeconstraints};
$\qkpo$ be defined by \cref{define_trivial_ellipsek} when $\activeconstraintsk = \emptyset$ and \cref{conservative_ellipsoid_details} otherwise; and $\minanglealpha$ be defined by \cref{minangleassumption}.
Suppose that \cref{minangleassumption} holds.
...
\end{align}
where $\minangledelta$ is defined by \cref{minangleassumption}.
\end{proof}
...
For all $x \in B\left(y^0, r\Delta\right)$,
\begin{align*}
...
\begin{proof}
Let $x \in B\left(y^{0}, r\Delta\right)$, and define $z = y^0 + \Delta\frac{x - y^0}{\left\|x - y^0\right\|}$.
...
Let $\minangledelta$ be defined by \cref{minangleassumption_alt},
and define
% such that if $\dk \le \minangledelta$ and
\begin{align}
\activeindicesk = \left\{i \in [m] \bigg| \zik \in B_{\infty}\left(\xk, \minangledelta \right) \right\} \label{define_activeindicesk}
\end{align}
...
\end{align*}
Thus, by \cref{minangleassumption_alt} and \cref{underbound} we know 
there exists a unit vector $\minangledirk\in\Rn$ and $k_2 \in \naturals$ such that 
for all $i \in \mathbb I^{(k)}$, $k \ge k_2$:
\begin{align*}
-\frac {\gmcik}{\left\|\gmcik\right\|} ^T\minangledirk \ge \minanglealpha \Longrightarrow \gmcik\left(-\minangledirk\right) \ge \minanglealpha \activegradmin
\end{align*}
...
\end{lemma}
\begin{proof}
If ${a}^T z' \le d$ and ${a'}^T z \le d$, then $z' - z = 0$.
Without loss of generality, suppose that ${a'}^T (z - p) > d$.
Then
\begin{align*}
0 < {a'}^T\left(z - p\right) - d 
...
\begin{align*}
\left[(1 - t) a + t a'\right]^T\left(z - p\right) - d 
= (1 - t) \left[a^T\left(z - p\right) - d\right] + t \left[{a'}^T\left(z - p\right) - d\right]
\le t \left[{a'}^T\left(z - p\right) - d\right] \\
\le \left\|a - a'\right\| \left\|y\right\|
\end{align*}
and
\begin{align*}
\left[(1 - t) a + t a'\right]^T(y - p) = (1 - t) a^T(y - p) + t {a'}^T(y - p) \ge \left(1-t\right) \delta\left\|y - p\right\| + t \delta\left\|y - p\right\| = \delta\left\|y - p\right\| \\
\Longrightarrow \frac 1 {\left[(1 - t) a + t a'\right]^T(y - p)} \le \frac {1} {\delta\left\|y - p\right\|}.
\end{align*}
Thus, by letting $\lambda = \left(\left\|y - p\right\| \delta\right)^{-1}\left\|a - a'\right\|\left\|y\right\|$, we see
\begin{align*}
\lambda
\ge \frac{\left[(1 - t) a + t a'\right]^T\left(z - p\right) - d}{\left[(1 - t) a + t a'\right]^T(y - p)} \\
\Longrightarrow \left[(1 - t) a + t a'\right]^T \left[z - \lambda \left(y - p\right) - p\right] \le d
\end{align*}
and
\begin{align*}
\left\|z - \lambda\left(y - p\right) - z\right\| \le  \delta^{-1}\left\|a - a'\right\|\left\|y\right\|.
\end{align*}
Thus,
\begin{align*}
...
\end{align*}
Also, if $i \in [m] \setminus J$ and $A_iy \ge b_i$, then
\begin{align*}
A_i \left(y - p\right) = A_i y - b_i + b_i - A_ip > \|A_i\| r 
\end{align*}
