...
Our algorithm maintains up to three trust regions:  \replace{}{a box-shaped outer trust region $\outertrk$,  an ellipsoidal inner trust-region $\sampletrk$ from which the sample points are chosen,  and a search trust-region, $\searchtrk$, which constrains the search for the next iterate.  }
The outer trust region is an $L_{\infty}$ ball of radius $ \dk $ defined by
\begin{align}
\outertrk = \tr = \{x\in \Rn | \; \xk_i - \dk \le x_i \le \xk_i + \dk \quad \forall 1\le i \le n\}. \label{define_outer_trust_region}
\end{align}
...
$\sampletrk \subset \outertrk \cap \replacemath{\feasible}{\feasiblek}$.
\replace{and $\xk \in \sampletrk $.    Of course,   it may only be the case that $\sampletrk \subset \feasiblek$ rather than $\sampletrk \subset \feasible$.}{     Since $\feasiblek$ is only an approximation of $\feasible$,  we place additional restrictions on  $\sampletrk$ to ensure that it is entirely contained within the feasible region $\feasible$.}   
\replace{However, we show that for sufficiently small $\dk$, this can not happen in Lemma 6.4.4.  }{These restrictions will be described in \cref{ellipsoid_requirements}}  
...
\replace{This means we introduce another trust region $ \searchtrk $ that also satisfies 
$ \searchtrk \subset \outertrk \cap \feasible$ and $\xk \in \searchtrk $ for the trust region subproblem.}{The third trust region is the search trust region, which  could either be $\sampletrk$ itself, or a larger region satisfying $ \sampletrk \subset \searchtrk \subset \outertrk \cap \feasible$.}  The search trust region is used in defining the trust region subproblem.
...
\end{lemma}
\begin{proof}
We are given a $\xinit \in \feasible$. Let $\maxgrad = 2\lipgrad \maxnorm + \max\left\{|\gradf\left(\xinit\right)|, \max_{1\le i\le m}\left\{\nabla c_i\left(\xinit\right)|\right\} \right\}$.
Thus, for any $x \in \Omega$,
\begin{align*}
\gradf(x) \le |\gradf\left(\xinit\right)| + \lipgrad \left\|x - 0 - \left(\xinit - 0\right)\right\| \le |\gradf\left(\xinit\right)| + 2\lipgrad \maxnorm \le \maxgrad.
\end{align*}
...
\subsubsection{Bound on Huffman Constant, with stronger assumption}

\begin{comment}
The first definition should be removed, or used everywhere.
\end{comment}

We make the following definitions:
\begin{align}
[m] = \left\{i \in \naturals | 1 \le i \le m \right\} \label{sha_m}\\
...
there is a unit vector $ \huffdirk \in \Rn $
such for any $i \in \activeprojkk \cup \activeprojkl$,
\begin{align*}
...
\end{lemma}
\begin{proof}
Let $i \in \activeprojkk \cup \activeprojkl$.
Let 
...
\le M \frac {\huffeps} {4M} - \gk^T \gmcil
\end{align*}
so that
\begin{align}
...
\le M \epsilon = \frac 1 {4} \huffeps.
\end{align*}
Combining this with \cref{til_replacement}, we see $-\gk^T \gmcik \ge - \frac 1 2 \huffeps$.
On the other hand, if $i \in \activeprojkk$, $-\gk^T \gmcik \ge 0 \ge - \frac 1 2 \huffeps$.
Either way, we have
\begin{align}
\label{til_either_way}
-\gk^T \gmcik \ge - \frac 1 2 \huffeps.
\end{align}
...
Suppose that the assumptions for \cref{bp_combining_several} hold.
...
That is,
\begin{align*}
\jackkl u_f^{(k,l)} \ge \frac 1 2 \huffalpha e.
\end{align*}

Let $J \subseteq \activeprojkk \cup \activeprojkl$ with $|J| \ne 0$ be arbitrary,
and suppose that $v_J \in \mathbb R^J_+$ with $\|v_J\| = 1$.
Then, Cauchy-Schwarz implies
 \begin{align*}
 \left\|\left(\jackj\right)^Tv\right\| = \left\|\left(\jackj\right)^Tv\right\| \left\|\huffdirk\right\| \ge
\left(\left(\jackj\right)^Tv\right)^T\huffdirk
= v^T\left(\jackj\right) \huffdirk
= \frac 1 2 \huffalpha v^Te
\ge \frac 1 2 \sqrt{|J|} \huffalpha
\ge \frac {\huffalpha} 2 .
\end{align*}
so that 
\begin{align*}
\left[ \left\|\left(\jackj\right)^Tv\right\|\right]^{-1} \le \frac 2 {\huffalpha}.
\end{align*}
Because $v$ was arbitrary, we have
\begin{align*}
\left[\min_{v \in \mathbb R^J_+, \|v\| = 1} \left\|\left( \jackj \right)^Tv\right\| \right]^{-1} \le \frac 2 {\huffalpha}.
\end{align*}
Because $J$ was arbitrary, we can substitute this into \cref{hoffman_theorem} to find
\begin{align*}
H\left(\jackkl\right) = \max_{J \in \mathcal S\left(\jackkl\right)} \left[\min_{v \in \mathbb R^J_+, \|v\| = 1}  \left\|\left(\jackj\right)^Tv\right\| \right]^{-1} \le \frac 2 {\huffalpha} = \huff.
\end{align*}
\end{proof}
...
\end{align}
Lastly, we assume that there is some $\hat x \in P_1$ and $h > 0$ such that 
\begin{align}
...
\label{4_1}
Suppose $A_1$ be an $m \times n$ matrix, $b_1, h \in \Rm$, and $\hat x \in \Rn$ such that \cref{bap_feasible_by_h} is satisfied.
Then for any $b' \in \Rm$ with $\|\left[b_1 - b'\right]^+\|_{\infty} \le \min_i h_i$, the system $A_1x \le b'$ is solvable.
\end{lemma}
...
\label{4_2}
Let $A_1, A_2 \in \mathbb R ^ {m \times n}$; $P_1, P_2 \subseteq \Rn$; $h, b_1, b_2 \in \Rm$; $\hat x \in \Rn$, $M, \epsilon, H\left(A_1\right) > 0$
be given satisfying
\cref{bap_define_the_polyhedra}, \cref{bap_feasible_by_h}, and \cref{bap_polyhedra_are_bounded}.
Let $H\left(A\right)$ be defined by \cref{hoffman_theorem}.
...
There exists $\epsilon_{h, 1}\left(h, m, H\left(A_1\right)\right)$ such that if 
$\epsilon \le \epsilon_{h, 1}\left(h, m, H\left(A_1\right)\right)$
then, for every $s_1 \in P_1$,
% satisfying $\epsilon'(1 + \|s_1\|) \le ?$ 
there corresponds an $s_2$ in $P_2$ satisfying 
$\|s_1 - s_2\|_{\infty}\le \epsilon H\left(A_1\right)\left(1 + \|s_1\|_{\infty}\right)$.
\end{lemma}
...
We define
\begin{align*}
\epsilon_{h, 1}\left(h, m, H\left(A_1\right)\right) \le \min\left\{\frac{\min_i h_i}{1 + M},\left(2 H\left(A_1\right)\right)^{-1}\right\}.
\end{align*}
...
By \cref{4_1}, this is solvable when
\begin{align*}
\|[b_1 - b_2 - (A_1 - A_2)x_n]^+\|_\infty \le \min_i h_i.
\end{align*}
This is satisfied because $\|[b_1 - b_2 - (A_1 - A_2)x_n]^+\|_{\infty} \le \epsilon + \epsilon\| x_n^+\|_{\infty} \le \epsilon(1 + \|x_n\|_\infty)$
and
\begin{align*}
...
\begin{align*}
\|x_{n+1} - x_n\|_\infty \le  H\left(A_1\right) \left\|[A_1x_n - b_2 - (A_1 - A_2)x_n]^+\right\|_\infty \\
\le \left\|[b_1 - b_2 - (A_1 - A_2)x_n]^+\right\|_\infty\le \epsilon(1 + \|x_n\|_\infty).
\end{align*}
For $n=1$ this reduces to
\begin{align*}
\|x_2 - x_1\|_\infty \le \epsilon H\left(A_1\right) (1 + \|s_1\|_\infty)
\Longrightarrow \|x_2\|_\infty \le \|s_1\| _\infty+ \epsilon H\left(A_1\right)(1 + \|s_1\|_\infty).
\end{align*}

For all other values of $n$, we also know that
\begin{align*}
\left\|x_{n+1} - x_{n}\|_\infty \le  H\left(A_1\right)\|[A_1x_n - b_2 - (A_1 - A_2)x_n]^+\right\|_\infty \\
\le  H\left(A_1\right)\left\|[b_2 + (A_1 - A_2)x_{n-1} - b_2 - (A_1 - A_2)x_n]^+\right\|_\infty
\le \epsilon  H\left(A_1\right)  \|x_{n}-x_{n-1}\|_\infty.
\end{align*}

By induction, we see that
\begin{align*}
\|x_{n+1} - x_n\|_\infty \le \left(\epsilon H\left(A_1\right)\right)^{n-1}\left\|x_2 - x_1\right\|_\infty \\
...
\begin{align*}
\|s_2 - s_1\|_\infty \le \frac{1}{1 - \epsilon H\left(A_1\right)}\|x_2 - x_1\| \le \frac{\epsilon H\left(A_1\right)}{1 - \epsilon H\left(A_1\right)}\left(1 + \|s_1\|_{\infty}\right) \le \epsilon H\left(A_1\right)\left(1 + \|s_1\|_{\infty}\right)
\end{align*}
...
\label{2_2}
Let $A_1, A_2 \in \mathbb R ^ {m \times n}$; $P_1, P_2 \subseteq \Rn$; $h, b_1, b_2 \in \Rm$; $\hat x,x_1^{\star}, x_2^{\star}  \in \Rn$, $M, \epsilon, H\left(A_1\right) > 0$
be given satisfying
\cref{bap_define_the_polyhedra}, \cref{bap_feasible_by_h}, \cref{bap_polyhedra_are_bounded}, and \cref{bap_define_the_projections}.
Let $H\left(A\right)$ be defined by \cref{hoffman_theorem}.

There exist
$\epsilon_{h, 2}\left(h, H\left(A_1\right), H\left(A_2\right), M\right) > 0$ and 
$M_{h, 2}\left(H\left(A_1\right), H\left(A_2\right), M\right) > 0$
such that if 
\begin{align*}
\epsilon \le \epsilon_{h, 2}\left(h, H\left(A_1\right), H\left(A_2\right), M\right),
\end{align*}
then
\begin{align*}
\|x_1^{\star} - x_2^{\star}\| \le M_{h, 2}\left(H\left(A_1\right), H\left(A_2\right), M\right) \sqrt{\epsilon}.
\end{align*}
\end{lemma}
...
\begin{align*}
\begin{array}{lrcl}
& \epsilon_{h, 2}\left(h, \huff\left(A_1\right), \huff\left(A_2\right), M\right) & = &
\min\left\{\frac 1 2, \frac{\min_i h_i}{1 + M},\left(2 H\left(A_1\right)\right)^{-1}, \left(2 H\left(A_2\right)\right)^{-1}\right\}\\
\textrm{and} \quad & M_{h, 2}\left(H\left(A_1\right), H\left(A_2\right), M\right) & = & \left( M' + \sqrt{2MM'}\right) \\
\textrm{where} \quad & M' & = &  4 H\left(A_2\right)\left(1 + M + 2 H\left(A_1\right)\left(1 + M\right)\right).
\end{array}
\end{align*}
...
c_{r_1} = 4 H\left(A_1\right)\left(1 + r_1\right) \\
r_2 =  \|x_1^{\star}\|_{\infty} + c_{r_1}\epsilon \\
c_{r_2}=  4 H\left(A_2\right)\left(1 + r_2\right).
\end{align*}
% Let $c_{r_1} = $
% We know that
...
\begin{align*}
r_1 \le M \\
c_{r_1} \le 4 H\left(A_1\right)\left(1 + M\right) \\
r_2 \le  M + 2 H\left(A_1\right)\left(1 + M\right) \\
c_{r_2} \le  4 H\left(A_2\right)\left(1 + M + 2 H\left(A_1\right)\left(1 + M\right)\right) = M'
\end{align*}
we see
\begin{align*}
...
Suppose that \cref{criteria_from_contradiction} holds.
The assumptions for \cref{accuracy_is_satisfied} hold.
Suppose that the assumptions for
\cref{lipschitz_gradients_assumption}, \cref{huffman_constant_is_bounded}
hold.
...
A set $S \subseteq\naturals \times \naturals$ is called \emph{Cauchy-squared} if
...
Let $\epsilon > 0$ be arbitrary.
There exists a $k_0$ such that if $k, l \ge k_0$ then
...
A_1 = \jackkl,
&
b_1 = \left[-c\left(\xk\right) + {\nabla m^{(k)}_c}^T\gk\right]_{\activeprojkk \cup \activeprojkl}, 
\\
A_2 = \jaclkl,
&
b_2 = \left[-c\left(\xl\right) + {\nabla m^{(l)}_c}^T\left(\xl - \xk + \gk\right)\right]_{\activeprojkk \cup \activeprojkl}.
\end{array}
\end{align*}
and let
\begin{align*}
P_1 = \left\{
x \in \Rn \bigg |
\mcik \left(\xk\right) + \gmcik \left(x - \gk \right) \quad \forall i \in \activeprojkk \cup \activeprojkl
\right\} \quad \textrm{and}\\
P_2 = \left\{
x \in \Rn \bigg |
\mcil \left(\xl\right) + \gmcil \left(x - \xl + \xk - \gk\right) \quad \forall i \in \activeprojkk \cup \activeprojkl
\right\}
\end{align*}
so that
\begin{align*}
...
With $s = x - \left(\xk - \gk \right)$, we see
\begin{align*}
\argmin_{s \in P_1} \left\|s\right\|
=
\begin{array}{ccc}
\argmin_{s \in \Rn} & \left\|s\right\| & \\
\textrm{s.t.} & \mcik \left(\xk\right) + \gmcik ^T\left(s - \gk\right) & \forall i \in \activeprojkk \cup \activeprojkl
\end{array} \\
=
\begin{array}{ccc}
\argmin_{x \in \Rn} & \left\|x - \left(\xk - \gk\right)\right\| & \\
\textrm{s.t.} & \mcik \left(\xk\right) + \gmcik ^T\left(x - \xk\right) & \forall i \in \activeprojkk \cup \activeprojkl
\end{array} \\
=
\begin{array}{ccc}
\argmin_{x \in \Rn} & \left\|x - \left(\xk - \gk\right)\right\| & \\
\textrm{s.t.} & \mcik \left(\xk\right) + \gmcik ^T\left(x - \xk\right) & \forall 1 \le i \le m
\end{array} \\
=
\argmin_{x \in \fik} \left\|x - \left(\xk - \gk\right)\right\| = \projk\left([m]\right).
\end{align*}
...
\begin{array}{ccc}
\argmin_{s \in \Rn} & \left\|s\right\| & \\
\textrm{s.t.} & \mcil \left(\xl\right) + \gmcil ^T\left(s - \xl + \xk - \gk\right) & \forall i \in \activeprojkk \cup \activeprojkl
\end{array} \\
=
\begin{array}{ccc}
\argmin_{x \in \Rn} & \left\|x - \left(\xk - \gk\right)\right\| & \\
\textrm{s.t.} & \mcil \left(\xl\right) + \gmcil ^T\left(x - \xl\right) & \forall i \in \activeprojkk \cup \activeprojkl
\end{array} \\
=
\begin{array}{ccc}
\argmin_{x \in \Rn} & \left\|x - \left(\xk - \gk\right)\right\| & \\
\textrm{s.t.} & \mcil \left(\xl\right) + \gmcil ^T\left(x - \xl\right) & \forall 1 \le i \le m
\end{array} \\
=
\argmin_{x \in \feasiblel([m])} \left\|x - \left(\xk - \gk\right)\right\| = \projkl\left([m]\right).
\end{align*}
...
H\left(\jackkl\right) \le \huff
\quad \textrm{and} \quad
H\left(\jaclkl \right) \le \huff.
\end{align*}
By \cref{constraints_are_convex}, we know that $\feasible \subseteq \fik$, so that $\xinit \in \fik$.
Thus, we can let $h = \xinit - \argmin_{x \not \in \feasible} \left\|\xinit - x\right\|$.
...
By \cref{model_gradients_are_cauchy}, we know that there is a $k_2 \in \naturals$ such that if $k, l \ge k_2$, then
$\left\|\gmcik - \gmcil\right\| \le \epsilon_1$
where
\begin{align*}
\epsilon_1 = 
\min\left\{
\epsilon_{h, 2}\left(h, \huff, \huff, M\right),
\left(\frac {\epsilon} {M_{h, 2}\left(\huff, \huff, M\right)}\right)^2
\right\}.
\end{align*}
...
respectively.
By \cref{model_gradients_are_cauchy} and \cref{criteria_from_contradiction}
there exists a $k_3 \in \naturals$ such that if $k, l \ge k_3$, then
\begin{align*}
\left\| \gmcik - \gmcil \right\| \le  \frac{\epsilon_1}{2\maxmodelgrad}
\quad \textrm{and} \quad
\left\|\xl - \xk\right\| \le  \frac{\epsilon_1}{2\left(\lipgrad + \maxmodelgrad\right)}
\end{align*}
...
\le
\maxmodelgrad \frac{\epsilon_1}{2\maxmodelgrad} + \left(\lipgrad + \maxmodelgrad\right) \frac{\epsilon_1}{2\left(\lipgrad + \maxmodelgrad\right)} \le \epsilon_1.
\end{align*}
Then, \cref{2_2} implies
\begin{align*}
\left\|
...
\right\|
\le
M_{h, 2}\left(\huff, \huff, M\right) \sqrt{\epsilon_1}
\le \epsilon.
\end{align*}

Thus, the result follows by letting $k_0 = \max\left\{k_1, k_2, k_3\right\}$.
\end{proof}
