...
\begin{align*}
x^{\star} = \text{Proj}_{\feasiblek}\left(x^{\star} - \nabla f(x^{\star})\right).
\end{align*}
...
For each $k \in \ints$, suppose we are given $n\times n$, symmetric, positive-definite matrices $\qk$, vectors $\ck \in \Rn$, scalars $\sdk > 0$, and a bound $\sigmamax > 0$.
Then the tuple $(\qk, \ck, \dk, \sigmamax)$ is suitable if each of the following hold:
\begin{itemize}
\item The condition numbers of $\qk$ are bounded:
\begin{align}
\sigma(Q^{(k)}) \le \sigmamax \quad \forall k \in \ints
\end{align}
...
\begin{align}
f_e(\epsilon, \delta, \theta; x) = (x - \epsilon e_1)^T\bigg(\begin{pmatrix}
1 & \boldsymbol0^T \\
\boldsymbol 0 & \frac{\theta^2}{1 - \theta^2} \boldsymbol I \\
\end{pmatrix}\bigg)(x - \epsilon e_1) - \frac 1 2 \delta^2 \label{define_ellipse_function}
\end{align}
and rotation matrix
\begin{align}
R = 2\frac{(e_1 + \huk)(e_1 + \huk)^T}{(e_1 + \huk)^T(e_1 + \huk)} - \boldsymbol I \label{define_rotation}
\end{align}
by defining
\begin{align}
...
\begin{align}
c_i(x) \ge c_i(y) + \nabla c_i(y)^T(x - y) \quad \forall x, y \in \Omega \\
...
Each function $c_i$ for $1 \le i \le m$ and $f$ has bounded hessian. That is, there exists a $ \maxhessian $ such that:
\begin{align*}
\|\nabla f(x) \| \le \maxhessian \quad \forall x \in \Omega \\
...
\begin{align}
\|\nabla f(x) - \nabla f(y)\| \le \lipgrad \|x - y\| \quad \forall x,y \in \Omega, \\
...
\begin{align}
\|\nabla f(x)\| \le \maxgrad \quad  \forall x \in \feasible \\
\|\nabla c_i(x)\| \le \maxgrad \quad  \forall x \in \feasible \forall 1 \le i \le m
\end{align}
\end{lemma}
\begin{proof}
We are given a $x^{(0)} \in \feasible$, let $\maxgrad = 2\lipgrad \maxnorm + \max\left\{|\nabla f(x^{(0)})|, \max_{1\le i\le m}\left\{\nabla c_i(x^{(0)})|\right\} \right\}$.
Thus, for any $x \in \Omega$,
\begin{align*}
\nabla f(x) \le |\nabla f(x^{(0)})| + \lipgrad \left\|x - 0 - (x^{(0)} - 0)\right\| \le |\nabla f(x^{(0)})| + 2\lipgrad \maxnorm \le \maxgrad.
\end{align*}
...
% Assume that \cref{lipschitz_gradient} and \cref{maxnormassumption} hold.
% Then, the function $f$ has bounded gradient in $ \Omega $. That is, there exists an $\gfmin > 0$ such that
% \begin{align}
...
% \theta^{\text{min}} = \liminf_{k\to\infty} \thetamink \label{def_theta_min} \\
...
\gamma = 1 + \frac 1 {\sqrt 2}
\end{align}
where $\dacc$ is defined in \cref{define_boundedbeta_deltasmall}, and $R$ be defined as in \cref{define_rotation}.
Then, \cref{ellipsoids_exist} is satisfied by letting
\begin{align*}
\qk = R^T \begin{pmatrix}
1 & \boldsymbol0^T \\
...
\end{align*}
as $\det(R) = 1$.
Also, $R\huk = e_1$, so that for any $\delta > 0$,
\begin{align*}
...
\end{pmatrix}\bigg) R\left(x - \left(\xk + \frac 1 2 \dk \huk\right)\right) - \frac 1 2 \delta^2 \\
= \left(R(x-\xk) - \frac 1 2 \dk e_1\right)^T\bigg(\begin{pmatrix}
1 & \boldsymbol0^T \\
\boldsymbol 0 & \frac{\left(\beta^{\star}\right)^2}{1 - \left(\beta^{\star}\right)^2} \boldsymbol I \\
\end{pmatrix}\bigg) \left(R(x-\xk) - \frac 1 2 \dk e_1\right) - \frac 1 2 \delta^2 \\
= f_e\left(\frac 1 2 \dk, \delta, \beta^{\star}; R\left(x - \xk\right)\right).
\end{align*}
In particular, letting $\delta = \sdk$ and $\delta = \sqrt{2}\sdk$, we see that
\begin{align*}
\unshiftedellipsoid = \left\{x \in \Rn | f_e\left(\frac 1 {2\gamma} \dk, \frac 1 {2\gamma} \dk, \beta^{\star}; R\left(x - \xk\right)\right) \le 0 \right\} \\
\scaledunshiftedellipsoid = \left\{x \in \Rn | f_e\left(\frac 1 {2\gamma} \dk, \sqrt{2} \frac {1}{2\gamma}\dk, \beta^{\star}; R\left(x - \xk\right)\right) \le 0 \right\}.
\end{align*}
...
\begin{align*}
\unshiftedellipsoid \subseteq \left\{t y \in \Rn | e_1^TR\left(y - \xk\right) \ge \beta^{\star}, \|R\left(y - \xk\right)\| = 1, t > 0 \right\} \\
= \left\{x \in \Rn | x = \xk + ts, e_1^TRs = s^T\huk \ge \beta^{\star}, \|s\| = \|Rs\| = 1, t > 0 \right\}
\end{align*}
as $R = R^T$.
We know two things from this, because $\beta^{\star} \ge \bsk$ and $\beta^{\star} \ge \frac 1 2$:
\begin{align*}
...
$f_e\left(\frac 1 2 \dk, \frac 1 {\sqrt{2}}\dk, \beta^{\star}; R\left(\xk - \xk\right)\right) = 0$, so that $\xk \in \scaledunshiftedellipsoid$.
\end{proof}
...
Assume that \cref{lipschitz_gradients_assumption} and \cref{bounded_hessians_assumption} hold, and that $\dk \le \dacc$.
Then, there exists a $\kappa_g$ such that
\begin{align*}
\|\nabla f(\xk) - \nabla \mfk(\xk) \| \le \kappa_g \dk^2 \\
\|\nabla c_i(\xk) - \gmcik \| \le \kappa_g \dk^2 \quad \forall 1 \le i \le m.
\end{align*}
...
After defining $\kappa''_{g} =  \sigmamax\kappa'_g$, we see that
...
\mathcal K = \big \{ k \in \ints | \dk \le \min \{ \dsr, \frac {\chik}{\hfb}, \frac{1-\eta_1}{c}\chik, \left(\frac 1 {\kappa_{\chi}}  \chik\right)^{\frac 1 {p_{\Delta}}}, 1 \} \big \} \label{define_mathcal_k}
\end{align}


\begin{comment}
The following lemma uses \cref{bounded_hessians_assumption} on the model functions....
\end{comment}
...
\cref{minangleassumption},
and \cref{bounded_below_assumption} hold.
...
By \cref{lipschitz_gradients_assumption}, \cref{bounded_hessians_assumption}, \cref{accuracy_is_satisfied},
\begin{align*}
...
\begin{align*}
\dk \le \left[\frac {\mingrad  \beta} {4\maxconstrainthessian\sqrt{n}\left(1 + \frac {\lpc} \maxconstrainthessian \right)}\right]^{\frac1 {1 - p_{\beta}} }
\Longrightarrow \sqrt{n}\left(1 + \frac {\lpc} \maxconstrainthessian \right) \dk^{1-p_{\beta}}\le \frac 1 {4\maxconstrainthessian} \mingrad  \beta
\end{align*}
Multiplying by $\dk^{p_{\beta}}$,
\begin{align*}
\sqrt{n}\left(1 + \frac {\lpc} \maxconstrainthessian \right) \dk \le \frac 1 {4\maxconstrainthessian} \mingrad  \beta \dk^{p_{\beta}}
\Longrightarrow \sqrt{n} \dk \le -\frac 1 \maxconstrainthessian \sqrt{n}\dk \lpc + \frac 1 {4\maxconstrainthessian} \mingrad  \beta \dk^{p_{\beta}}
\end{align*}
which implies by \cref{z_is_active}, \cref{t_is_bounded}, and \cref{nsc_pos} that
\begin{align*}
t 
\le \sqrt{n} \dk 
\le -\frac 1 \maxconstrainthessian \sqrt{n}\dk \lpc + \frac 1 {4\maxconstrainthessian} \mingrad \beta \dk^{p_{\beta}}
\le -\frac 1 \maxconstrainthessian \sqrt{n}\dk \lpc -\frac 1 \maxconstrainthessian \nabla c_i(\xk)^Ts.
\end{align*}
Multiplying by $\maxconstrainthessian$ and adding the right hand side, we see
\begin{align*}
\Longrightarrow \sqrt{n}\dk \lpc + \nabla c_i(\xk)^Ts + \maxconstrainthessian t \le 0.
\end{align*}
Multiplying by $t$ and using \cref{z_is_active},
\begin{align*}
 t \left(\lpc\|\wik - \xk\| + \nabla c_i(\xk)^Ts + \maxconstrainthessian t\right) \le t \left(\sqrt{n}\dk \lpc + \nabla c_i(\xk)^Ts + \maxconstrainthessian t\right) \le 0.
\end{align*}
...
\begin{align*}
c_i(y) = c_i(\wik + ts) + c_i(\wik) + t\nabla c_i(\wik)^Ts + \maxconstrainthessian t^2 \le 0.
\end{align*}
...
We will define the following for the next three proofs:

\begin{align}
\mathcal F^{(k)} = \{ y \in \Rn | c_i(\xk) + \gmcik^T(y - \xk) \le 0 \quad \forall 1 \le i \le m\} \label{def_two_proofs_fk} \\
p^{(k)} = \argmin_{p \in \mathcal F^{(k)}} \|\xk - \gk - p\|^2 \label{def_two_proofs_pk} \\
\mathcal A^{(k)} = \{1 \le i \le m | c_i(\xk) + \gmcik^T(p^{(k)} - \xk) = 0 \} \label{def_two_proofs_activeidx}
\end{align}
...
Assume that \cref{accuracy_assumption}, \cref{minactivegradassumption}, \cref{bounded_gradient_constraint} and \cref{bounded_gradient} hold as well as
\begin{align*}
\dk \le \min\left\{\minactivegraddelta, \sqrt{\frac{\minactivegraddelta}{2\gfmin\kappa_g}}\right\}.
\end{align*}
Use definitions \cref{def_two_proofs_fk}, \cref{def_two_proofs_pk}, and \cref{def_two_proofs_activeidx}, and let $\mathcal F_x, p_x, \mathcal A_x$ be defined as in \cref{define_projection_active}.
...
\Longrightarrow |c_i(\xk) + \nabla c_i(\xk)^T(p_{\xk} - \xk)| \le 2\gfmin \kappa_g \dk^2 \le \minactivegraddelta.
\end{align*}
\end{proof}
...
\cref{minactivegradassumption}, \cref{accuracy_assumption}
...
and using definitions \cref{def_two_proofs_fk}, \cref{def_two_proofs_pk}, and \cref{def_two_proofs_activeidx} also define
\begin{align}
...
\frac{2}{2\minactivegrad^2} \max\left\{\gcmax, c_i(\xk), c_i(x^{(l))}\right\}\max\left\{\lpc\left\|\xk - x^{(l)}\right\| + 4\kappa_{g}\dk^2, \lpc \|\xk - x^{(l)}\| + \maxconstrainthessian\|\xk - x^{(l)}\|^2\right\} \le \epsilon_1 \\
...
We have by \cref{accuracy_assumption}, that there exists a $\kappa_{g}$ such that $u^{(i)}, v^{(i)}\in\Rn$ such that 
\begin{align}
\|u^{(i)} \| \le 1, \quad 
\|u^{(i)} \| \le 1 \\
...
and $\left|c_i(\xk) - c_i(x^{(l)})\right| \le \lpc \|\xk - x^{(l)}\| + \maxconstrainthessian\|\xk - x^{(l)}\|^2$ by \cref{lipschitz_constraints} and \cref{bounded_constraint_hessians},
\begin{align*}
\left|\left\|\nabla m_{c_i}^{(l)}\left(x^{(l)}\right) \right\|c_i(\xk) - \left\|\gmcik\right\|c_i(x^{(l)})\right| \\
\le 2 \max\left\{\gcmax, c_i(\xk), c_i(x^{(l))}\right\}\max\left\{\lpc\left\|\xk - x^{(l)}\right\| + 4\kappa_{g}\dk^2, \lpc \|\xk - x^{(l)}\| + \maxconstrainthessian\|\xk - x^{(l)}\|^2\right\}
\end{align*}
so that
Get to
\begin{align*}
\left|\frac{c_i(\xk)}{\left\|\gmcik\right\|} - \frac{c_i(x^{(l)})}{\left\|\nabla m_{c_i}^{(l)}\left(x^{(l)}\right) \right\|}\right| \\
\le \frac{2}{2\minactivegrad^2} \max\left\{\gcmax, c_i(\xk), c_i(x^{(l))}\right\}\max\left\{\lpc\left\|\xk - x^{(l)}\right\| + 4\kappa_{g}\dk^2, \lpc \|\xk - x^{(l)}\| + \maxconstrainthessian\|\xk - x^{(l)}\|^2\right\}
\le \epsilon_1.
\end{align*}
Thus, we can apply \cref{3_9} to find that
\begin{align*}
\left\|p^{(k)} - p^{(l)} \right\| \le \epsilon
\end{align*}
