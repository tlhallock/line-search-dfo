...
\begin{itemize}
\item The tuples $(\qk, \ck, \dk, \sigmamax)$ are \emph{bounded} if the condition numbers of $\qk$ are bounded:
\begin{align}
\sigma(Q^{(k)}) \le \sigmamax \quad \forall k \in \naturals \label{define_suitable_condition_numbers}
\end{align}
\item The tuple $(\qk, \ck, \dk, \sigmamax)$ is \emph{trusted} if an ellipsoid defined by $\qk, \ck, \sdk$ lies within the trust region:
\begin{align}
...
\end{align}
\item The tuple $(\qk, \ck, \dk, \sigmamax)$ is \emph{feasible} if an ellipsoid defined by $\qk, \ck, \sdk$ lies within the feasible region:
\begin{align}
\unshiftedellipsoid \subseteq \feasible.
\end{align}
where $\unshiftedellipsoid$ is defined by \cref{define_unshifed_ellipsoid}
\item The tuple $(\qk, \ck, \dk, \sigmamax)$ is \emph{adjacent} if an ellipsoid defined by $\qk, \ck, \sdk$ is near to the current iterate:
\begin{align}
...
\begin{comment}
Need to move these definitions....
\end{comment}
...
\begin{align*}
\nabla c_i(x)^T\minanglediralt(x) > \minanglealpha\quad \forall i \in \epsactive
\end{align*}
\end{assumption}
...
Fix an iterate $k$.
Suppose that $\qk$, $\ck$, and $\sdk$ are non-empty and bounded according to \cref{ellipsoids_notation_definitions}.
...
Then the transformation $T(x) = \delta D L^T(x - \mu^{(k)})$ maps $E^{(k)}$ to the $\delta$ ball.
...
Notice that because $\sigma_{\textrm{min}} = 1$, we have that $\sigmamax = \kappa\left(\qk\right)$.
...
Let $\minanglealpha$ and $\minangledelta$ be defined by \cref{minangleassumption} and define $\thetamink$ by \cref{define_thetamink}.
...
\begin{lemma}
\label{boundbeta}
Let $\bsk$ and $\fcki$ be defined as in \cref{define_bsk} and \cref{define_inner_cone}, as well as
$\minanglealpha$ be defined as in \cref{minangleassumption}.
If \cref{minangleassumption} holds,
then there exists a $\dacc > 0$ such that $\bsk \le 1 - \frac 1 4 (\minanglealpha)^2$ whenever $\dk \le \dacc$.
\end{lemma}
...
\begin{align}
\frac {1} 4 \le \left(\bs\right)^2 \le \left(1 - \frac 1 4 \left(\minanglealpha\right)^2\right)^2 \le 1 - \frac 1 4 \left(\minanglealpha\right)^2 \label{p2_numerator}
\end{align}
so that
\begin{align}
\frac 1 4 \left(\minanglealpha\right)^2 \le 1 - \left(\bs\right)^2 \le \frac 3 4. \label{p2_denominator}
\end{align}
