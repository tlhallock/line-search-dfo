...
Also, by \cref{def_f} we know that $-s^T \hgik \ge \beta \dk^{\frac 1 2}$ where $\|s\| = 1$, and by \cref{def_g_bounds}, \cref{def_lambda_poised}, and our assumption \cref{delta_is_small_enough}, we know
...
u_c = P_{\capcones\cap\tr}(\xk-\gk) \\
u_{GC} = P_{\linearization\cap\tr}(\xk-\gk) \\
\delta = \frac 1 {2\gfmin} \kappa_f \kappa_{\chi} \min\left\{ \frac{\kappa_{\chi}}{\hfb}, 1 \right\} \label{sr_define_delta}
\end{align}
and let $\kappa_{\chi} > 0$ be a constant.
Then, by letting $\kappa_{r} > 0$ be defined by
\begin{align}
\kappa_r = \min\{\frac 1 2 , \}
\end{align}
we know that if, during iteration $k$, we have
\begin{align}
\chik \ge \kappa_{\chi} \dk^{\frac 1 2} \label{sr_chi_big_enough}
\end{align}
and
\begin{align}
\dk \le \min\left\{1, \minangledelta, \left(\frac 1 2 \frac{\delta\minanglealpha^2\min\{\kappa_{\chi}, 1 \}}{\sqrt{2}\left(2\beta + \alpha\right)}\right)^2 \right\}, \label{sr_delta_small_enough}
\end{align}
we also have
\begin{align*}
\left(-\gk\right)^T \left(u_c - \xk\right) \ge \kappa_r \left(-\gk\right)^T \left(u_{GC} - \xk\right) \\
\end{align*}
...
Consider the point 
\begin{align}
v = u_{GC} - \delta \dk^{\frac 1 2} \left(\left(u_{GC}- \xk\right)^T u\right)u. \label{define_v}
\end{align}
...
\left(-\gk\right)^T\left(u_{GC} - \xk\right) 
 ...
 \ge \kappa_f \kappa_{\chi} \dk^{\frac 1 2} \min\left\{ \frac{\kappa_{\chi} \dk^{\frac 1 2}}{1+\|\nabla^2 \mfk(\xk)\|}, \dk \right\}
 \ge \kappa_f \kappa_{\chi} \dk^{\frac 3 2} \min\left\{ \frac{\kappa_{\chi}}{\hfb}, 1 \right\}.
\end{align*}
Using this and \cref{sr_define_delta}, we know that
\begin{align*}
\delta \dk^{\frac 1 2}  \left\|\gk\right \|  \left|\left(u_{GC}- \xk\right)^T u\right| \le \delta\gfmin  \dk^{\frac 3 2}  \\
\le \frac 1 2 \kappa_f \kappa_{\chi} \dk^{\frac 3 2}\min\left\{ \frac{\kappa_{\chi}}{\hfb}, 1 \right\}
...
\ge\left(-\gk\right)^T\left(u_{GC} - \xk\right)  - \delta \dk^{\frac 1 2} \left\|\gk\right \|  \left|\left(u_{GC}- \xk\right)^T u\right|  \\
...
\end{align}
However, because $u_{GC}$ is feasible with respect to the linearization of the constraints, we also know that
\begin{align}
\left(\zik - \xk\right)^T(u_{GC} - \xk) \le \left\|\zik - \xk\right\|^2. \label{gc_is_feasible}
\end{align}


If $u_l \in \tr$, then by \cref{sr_chi_big_enough}: $\left\|u_{GC} - \xk\right\|  = \left\|u_{l} - \xk\right\| = \chik \ge \kappa_{\chi} \dk^{\frac 1 2} \ge  \kappa_{\chi} \dk$.
...
=\left( u_{GC} - \delta\dk^{\frac 1 2} \left(\left(u_{GC} - \xk\right)^T u\right)u - \xk - \left(1 - \alpha\dk^{\frac 3 2}\right)\left(\zik - \xk\right) \right)^T\left(\xk - \zik \right) \\
=\left[
-\left( u_{GC}- \xk\right)^T\left(\zik - \xk \right) 
+\left(1 - \alpha\dk^{\frac 3 2}\right)\left\|\zik - \xk\right\|^2
\right]
- \delta\dk^{\frac 1 2}\left[ \left(u_{GC} - \xk\right)^T u\right]\left[ \left(\xk - \zik \right)^Tu\right]
\end{align*}
...
+ \frac 1 2 \delta\dk^{\frac 1 2}\minanglealpha^2 \left\|u_{GC} - \xk\right\| \|\zik - \xk\| \\
...
\Longrightarrow \left(v - \wik \right)^T\left(\xk - \zik \right)\ge\beta \dk^{\frac 1 2} \left\|v - \wik\right\| \left\| \xk - \zik\right\| \\
...
% 
% \begin{align}
% -\frac{\left(v - \xk\right)^T}{\left\|v - \xk\right\|}\frac{\gmcik}{\left\|\gmcik \right\|} \ge \alpha \dk^{\frac 1 2}
% \end{align}
% 
% % = \left[\left(u - \xk\right)^T\gk+ \frac{\left\|\gk\right\|^2 - \left(u - \xk\right)^T\gk}{\left(\xk\right)^T\gk}\left(\left(\xk\right)^T\gk - \left\|\gk\right\|^2\right) \right] \\
% 
% 
% 
...
% \begin{align}
% u + z \left(\xk - \gk - u\right) \\
% \left[ u - \xk + z \left(\xk - \gk - u\right) \right] ^T \gk = 0\\
% \left( u - \xk\right)^T\gk - z \left(\gk + \left(u - \xk\right) \right) ^T \gk = 0\\
% z \left(\left\|\gk\right\|^2 + \left(u - \xk\right)^T\gk \right) = \left( u - \xk\right)^T\gk\\
% z \left(\frac {\left\|\gk\right\|^2}{ \left( u - \xk\right)^T\gk} + 1 \right) = 1\\
% z = \left(\frac {\left\|\gk\right\|^2}{ \left( u - \xk\right)^T\gk} + 1 \right)^{-1}\\
% \end{align}
...
\begin{theorem}
Suppose that Hypothesis \cref{lipschitz_gradient},  \cref{bounded_hessians}, and \cref{bounded_below} hold. Then $\mathcal K \subset \bar{S}$.
\end{theorem}
