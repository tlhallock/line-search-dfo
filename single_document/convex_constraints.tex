...
\begin{itemize}
\item The tuples $(\qk, \ck, \dk)$ are \emph{bounded} if the condition numbers of $\qk$ are bounded: there exists a $\sigmamax \ge 1$ with
\begin{align}
\sigma(Q^{(k)}) \le \sigmamax \quad \forall k \in \naturals \label{define_suitable_condition_numbers}
\end{align}
\item The tuple $(\qk, \ck, \dk)$ is \emph{trusted} if an ellipsoid defined by $\qk, \ck, \sdk$ lies within the trust region:
\begin{align}
...
\end{align}
\item The tuple $(\qk, \ck, \dk)$ is \emph{feasible} if an ellipsoid defined by $\qk, \ck, \sdk$ lies within the feasible region:
\begin{align}
\unshiftedellipsoid \subseteq \feasible.
\end{align}
where $\unshiftedellipsoid$ is defined by \cref{define_unshifed_ellipsoid}
\item The tuple $(\qk, \ck, \dk)$ is \emph{adjacent} if an ellipsoid defined by $\qk, \ck, \sdk$ is near to the current iterate:
\begin{align}
...
\end{align}
\item The tuple $(\qk, \ck, \dk)$ are \emph{non-empty} if
\begin{align}
\unshiftedellipsoid \ne \emptyset
\end{align}
where $\unshiftedellipsoid$ is defined by \cref{define_unshifed_ellipsoid}.
...
Here we show how to construct the ellipsoid, and that it satisfies \cref{ellipsoids_notation_definitions}.
...
\sampletrk = \unshiftedellipsoid = \left\{x \in \Rn | f_e\left(\frac 1 {2\gamma} \dk, \frac 1 {2\gamma} \dk, \max\left\{\frac 1 2 , \bsk\right\}; \rotk(x - \xk)\right) \le 0\right\}. \label{define_ellipsek}
\end{align}
...
\end{align*}
By moving the second and third terms over, we find
\begin{align}
...
\begin{lemma}
\label{boundbeta}
Let $\bsk$ and $\fcki$ be defined by \cref{define_bsk} and \cref{define_inner_cone}, as well as
$\minanglealpha$ be definedby \cref{minangleassumption}.
If \cref{minangleassumption} holds,
then there exists a $\dacc > 0$ such that $\bsk \le 1 - \frac 1 4 (\minanglealpha)^2$ whenever $\dk \le \dacc$.
\end{lemma}
...
Also, let $\minangledelta$ be defined by \cref{minangleassumption}.
Then we can let $\dacc$ be defined by
\begin{align}
\dacc =\min\left\{
1,
\minangledelta,
...
\right\}\label{define_delta_accuracy}.
\end{align}
If $\dk \le \dacc$, then
\begin{align}
...
Let $\activeconstraintsk$, and $\dacc$ be defined by \cref{define_activeconstraints}, and \cref{define_delta_accuracy} respectively.
If $\activeconstraintsk \ne \emptyset$ and $\dk \le \dacc$, then the ellipsoids defined by \cref{define_ellipsek} are trusted and adjacent according to \cref{ellipsoids_notation_definitions}.
...
For \cref{define_suitable_close_to_iterate}, we have \cref{ellipse_fits} tells us that
...
$\thetamink \le \epsilon$ and $\dk \le \dacco(\epsilon)$, we have
...
For any $0 < \epsilon < 1$ there exists $\dacco(\epsilon) > 0$, such that for any $k \in \naturals$ with
$\thetamink \le \epsilon$ and $\dk \le \dacco(\epsilon)$, we have $\sigma(\qk) \le \frac{12}{\epsilon^2}$.
\end{lemma}
...
\end{theorem}
\begin{proof}
This is a direct consequence of \cref{accuracy_is_satisfied_lemma}, \cref{ellsoid_is_suitable_theorem_p1}, and \cref{ellsoid_is_suitable_theorem_p1}.
\end{proof}
...
In this section we show that ellipsoids are bounded as in \cref{ellipsoids_notation_definitions}.
Notice that this is required in the proof of \cref{accuracy_is_satisfied}.
Although \cref{minangleassumption} can be proven from \cref{minangleassumption_alt} as done within \cref{minanglelemmap2},
the proof also requires \cref{accuracy_is_satisfied}.
To circumvent this circular dependency, we introduce \cref{minangleassumption}.

\begin{comment}
We believe this is not actually needed.
\end{comment}

\begin{assumption}
\label{minangleassumption}
There exists $\minangledelta > 0$ and $\minanglealpha > 0$ such that for each $x \in \Omega$ there exists a $\minangledir(\xk) \in \Rn$ with $\|\minangledir(\xk)\| = 1$ such that 
...
% 
% \begin{lemma}
% Let $\minanglealpha$ and $\minangledelta$ be defined by \cref{minangleassumption} and define $\fcki$ by \cref{define_inner_cone}.
% Assume that \cref{minangleassumption} holds.
% If $\dk \le \minangledelta$, then $\fcki \ne \emptyset$.
% \end{lemma}
% 
% \begin{proof}
% This follows directly from \cref{theta_min_is_bounded} and \cref{define_inner_cone}.
% \end{proof}
...
\begin{lemma}
\label{ellsoid_is_bounded_lemma}
Let $\activeconstraintsk$, and $\dacc$ be defined by \cref{define_activeconstraints}, and \cref{define_delta_accuracy} respectively.
Suppose that \cref{minangleassumption} holds.
If $\activeconstraintsk \ne \emptyset$ and $\dk \le \dacc$, then the ellipsoids defined by \cref{define_ellipsek}
are bounded according to \cref{ellipsoids_notation_definitions}.
\end{lemma}
\begin{proof}
...
\begin{lemma}
\label{this_is_probably_not_worthy_of_being_a_lemma}
Suppose that $x\in\reals$ is such that $0 \le x \le 1$.
Then $1 - x \le \sqrt{1-x^2}$.
\end{lemma}
\begin{proof}
We can multiply by $2x$ to get
$0 \le 2x^2 \le 2x$,
and add $1 - 2x - x^2$ to find
$1 - 2x + x^2 = (1 - x)^2 \le 1-x^2$.
The result is the square root of these positive numbers.
\end{proof}

\begin{lemma}
\label{bound_bsk_by_theta_lemma}
Let $\beta$, $\thetamink$, and $\bsk$
be defined by 
\cref{define_alpha_beta}, \cref{define_thetamink}, and \cref{define_bsk}
respectively.
There exists $\dacc > 0$ such that for any $\epsilon > 0$, if $\thetamink \le \epsilon$ and $\dk \le \dacc$, then $\bsk \le 1 - ?$.
\end{lemma}
\begin{proof}
By defining $\dacc$ to be 
\begin{align}
\dacc = \left(\frac 1 {4\beta}\right)^{\frac 1 {p_{\beta}}}
\end{align}
we see that 
\begin{align*}
\beta \dk^{p_{\beta}} \le \frac 1 4
\end{align*}
whenever $\dk \le \dacc$.
This means that 
\begin{align*}
...
% Let
% be defined by
% \cref{define_epsactive} and \cref{define_epsactivemodels}
% respectively.
% Then $\epsactivemodels(x; \epsilon) \subseteq \epsactive(x; \epsilon)$.
% \end{lemma}
% \begin{proof}
% \end{proof}
% 
...
\begin{comment}
Is $\|\cdot\|_0$ the dual norm of $\|\cdot\|_{\infty}$?
\end{comment}
