...
tells us there is a $\minangleu$ with $-\frac {\gmcik}{\|\gmcik\|} ^T\minangleu \ge \minanglealpha$ for any $i \in \activeconstraintsk$.
...
\ge \left|2\left(\gk\right)^T(u_{GC} - v) + 4\left(u_{GC} - v\right) ^T\left(\nabla^2m_f^{(k)}\left(\xk\right)\right)\left(u_{GC} - v\right) \right|\\
