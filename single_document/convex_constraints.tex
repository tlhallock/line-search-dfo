...
\mathcal A^{(k, l)} = \{1 \le i \le m | c_i(\xk) + \gmcik^T(p^{(k, l)} - \xk) = 0 \}. \label{def_two_proofs_activeidx}
\end{align}
...
Using the definition of $\chik$, the triangle inequality, the contraction property of projections, \cref{chi2zero2_comp1},
...
So that
\begin{align}
\frac{\epsilon} 2 \le \left(2 + \lipgrad\right) \|\xk - x^{(l_k)}\| + \kappa_{g}\left(\dk + \Delta_{l_k}\right) + \| p^{(k)} - p^{(k_l)}\| \label{chi2zero2_conv}.
\end{align}
