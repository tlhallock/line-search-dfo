...
\xk - \frac{m^{(k)}_{c_i}\left(\xk\right)}{\left\|\gmcik\right\|^2} \gmcik & c_i\left(\xk\right) \ne 0 \quad \textrm{and} \quad \left\|\gmcik\right\| \ne 0 \\
\infty & c_i\left(\xk\right) \ne 0 \quad \textrm{and} \quad \left\|\gmcik\right\| = 0 \\
\end{cases}
\end{align}
...
Suppose that 
\cref{bounded_hessians_assumption}, \cref{lipschitz_gradients_assumption}, \cref{lipschitz_hessians_assumption}, and \cref{minangleassumption_alt} hold.
There exist $\kappa_g>0$ and $\kappa_h>0$ such that
\begin{align*}
\begin{array}{ccc}
...
Suppose that \cref{bounded_hessians_assumption}, \cref{lipschitz_gradients_assumption}, \cref{lipschitz_hessians_assumption}, and \cref{minangleassumption_alt} hold.
...
\cref{lipschitz_hessians_assumption},
...
By \cref{liminf_chi_to_zero}, we are able to define the sequence of pairs
\begin{align}
\label{define_slb}
S_{\textrm{lb}} = \left\{\left(k,
\min_{i \in \naturals, i > k, \chi_{i} \le \frac{\epsilon_{\textrm{lb}}}2} i
\right)\right\}_{k=1}^{\infty}.
\end{align}

Using the same constants as in \cref{define_reduceiterates}, we define
\begin{align}
\Delta_{\textrm{lb}}\left(\epsilon_{\textrm{lb}}\right) = \min\left\{
\dsr,
\frac{\epsilon_{\textrm{lb}}}{\maxmodelhessian}, 
\frac{(1-\gammasm)}{\theirc}\epsilon_{\textrm{lb}}, 
\left(\frac 1 {\kappa_{\chi}}  \epsilon_{\textrm{lb}} \right)^{\frac 1 {p_{\Delta}}}, 
1
...
\begin{align}
\mathcal U(\epsilon_{\textrm{lb}}) = \left\{k \in \naturals | \chik \ge \epsilon_{\textrm{lb}} \right\} \label{define_the_finite_set}
\end{align}
is infinite.
...
Let $\mathcal U\left(\epsilon_{\textrm{lb}} \right)$ and $S_{\textrm{lb}}$, be defined by \cref{define_the_finite_set} and \cref{define_slb}.
...
If there exists an $\epsilon_{\textrm{lb}} > 0$ such that $\mathcal U\left(\epsilon_{\textrm{lb}} \right)$ is infinite, then
\begin{align*}
\lim_{(k, l) \in S_{\textrm{lb}}, k \to \infty} \left\|\xk - \xl\right\| = 0.
\end{align*}
...
then $\dk \le \Delta_{\textrm{lb}} = \Delta_{\textrm{lb}}(\epsilon_{\textrm{lb}})$ as defined in \cref{define_delta_lb}.
Let $\miniterates$ be defined by \cref{define_miniterates}, and consider
$C_k = \left\{i \in \miniterates | k_1 \le k \le i < l \right\}$ for $(k, l) \in S_{\textrm{lb}}$.
Note that $k \in \miniterates$, so $C_k \ne \emptyset $.
For each $i \in C_k$, using the fact that $i \in \miniterates$, \cref{bounded_model_hessian_lemma}, and \cref{sufficient_reduction_theorem}, we can conclude
\begin{align*}
f\left(x^{(i)}\right) - f\left(x^{(i+1)}\right) 
\ge \gammasm \left( \mfk\left(x^{(i)}\right) - \mfk\left(x^{(i)} + s^{(i)}\right) \right) 
\ge \gammasm \kappa_f \chi_i \min\left\{\frac{\chi_{i}}{\maxhessian}, \Delta_i, 1\right\} 
\end{align*}

By the definition of $l$, we have that $\chi_i > \frac 1 2 \epsilon_{\textrm{lb}}$ for all $i \in C_k$.
As $i \ge k$, we have $\Delta_i \le \frac{\epsilon_{\textrm{lb}}}{\maxhessian}$ and $\Delta_i \le 1$.
Therefore,
\begin{align*}
\frac{\Delta_i}{2} \le \frac{\epsilon_{\textrm{lb}}}{2 \maxhessian} \le \frac{\chi_i}{\maxhessian}.
\end{align*}

It follows that
\begin{align*}
f\left(x^{(i)}\right) - f\left(x^{(i+1)}\right) > \frac{\gammasm \kappa_f \epsilon_{\textrm{lb}} \Delta_i}{4}
\end{align*}

and hence
\begin{align*}
\Delta_i < \frac{4}{\gammasm \kappa_f \epsilon_{\textrm{lb}}} \left[ f\left(x^{(i)}\right) - f\left(x^{(i+1)}\right)\right].
\end{align*}
...
\begin{align}
\left\|\xk - \xl\right\| < \frac{4}{\gammasm \kappa_f \epsilon_{lb}} \left[ f\left(\xk\right) - f\left(\xl\right)\right].
\end{align}
...
If $g \in S \cap S'$, then $\left\|P_S(g) - P_{S'}(g)\right\|cd  = 0 \le 8 \sqrt{\epsilon} \le 8\left(1 + \|P_{S \cap S'}(g) - g\|\right) \sqrt{\epsilon}$.
Otherwise, let 
\begin{align*}
\begin{array}{ccc}
