...
\begin{align*}
\delta_2 \dk^{1 + p_{\Delta}} = \left(2\maxmodelgrad + 4\maxmodelhessian\right)\delta\dk^{1 + p_{\Delta}},
\end{align*}
...
\nu    &=& \frac 1 2 \epsilon\delta_{\alpha}(r+2)^{-1}(r + 1)^{-1}, &
\delta &=& \min\bigg\{
1,
\gamma^{-1},
...
Notice that $\delta \le \nu = \frac{\epsilon\delta_{\alpha}}{2(r+2)(r + 1)} \Longrightarrow s \le \frac{\epsilon}{4 + 2r - \epsilon}$, which implies two things.
...
\begin{align}
\mathbb A_1^{(k)} &= \left\{ i \in [m] \bigg | c_i\left(\xk\right) + \gmcik^T\left( P_{\feasiblek}\left(\xk - \gk\right) - \xk\right) = 0\right\}, \label{tbbt_a1} \\
\mathbb A_2^{(k)} &= \left\{ i \in [m] \bigg | c_i\left(\xl\right) + \gmcil^T\left( P_{\feasiblel}\left(\xk - \gk\right) - \xl\right) = 0\right\}, \label{tbbt_a2} \\
\mathbb A_3^{(k)} &= \left\{ i \in [m] \bigg | c_i\left(\xk\right) + \nabla c\left(\xk\right)^T\left( P_{\truefeasiblek}\left(\xk - \gk\right) - \xk\right) = 0\right\}. \label{tbbt_a3}
\end{align}

\begin{lemma}
\label{norm_of_active_constraints_bounded_below}
For some fixed $k$, suppose that $\mathbb A_1^{(k)}$, $\mathbb A_2^{(k)}$, $\mathbb A_3^{(k)}$
are defined by
\cref{tbbt_a1}, \cref{tbbt_a2}, and \cref{tbbt_a3}.
Suppose that a set $S$ satisfies \cref{criteria_from_contradiction}.
There exists $\delta > 0$ and $k_0 \in \naturals$ such that,
\begin{itemize}
\item if $k \ge k_0$, then if $i \in \mathbb A_1^{(k)} \cup \mathbb A_3^{(k)}$, we have
\begin{align*}
\left\|\gmcik\right\| \ge \delta \quad \textrm{and} \quad
\left\|\nabla c_i\left(\xk\right) \right\| \ge \delta.
\end{align*}
\item if $k, l \ge k_0$ and $(k, l) \in S$, then if $i \in \mathbb A_1^{(k)} \cup \mathbb A_2^{(k)}$, we have
\begin{align*}
...
\paragraph{Case 2}
Suppose that $-c_i\left(\xl\right) > \mingradepsilon$ and $i \in \mathbb A_1^{(k)}$.
\begin{align*}
...
\paragraph{Case 3}
Suppose that $-c_i\left(\xl\right) > \mingradepsilon$ and $i \in \mathbb A_2^{(k)}$.
\begin{align*}
...
\paragraph{Case 4}
Suppose that $-c_i\left(\xl\right) > \mingradepsilon$ and $i \in \mathbb A_3^{(k)}$.
\color{red}
...
and \cref{model_gradients_are_cauchy}
are satisfied.
...
\end{align*}
Notice that if $\mathbb A_1^{(k)}$, $\mathbb A_2^{(k)}$, $\mathbb A_3^{(k)}$
are defined by
\cref{tbbt_a1}, \cref{tbbt_a2}, and \cref{tbbt_a3},
the result immediately follows from showing that
\begin{align*}
\left\|P_{\feasiblek\left(\mathbb A_1^{(k)}\right)}\left(\xk - \gk\right) - P_{\truefeasiblek\left(\mathbb A_3^{(k)}\right)}\left(\xk - \gk\right)\right\| \le \epsilon \\
\left\|P_{\feasiblek\left(\mathbb A_1^{(k)}\right)}\left(\xk - \gk\right) - P_{\feasiblel\left(\mathbb A_2^{(k)}\right)}\left(\xk - \gk\right)\right\| \le \epsilon.
\end{align*}
Further, notice that by \cref{i_thought_i_proved_this_already} and \cref{delta_to_zero}, for sufficiently large $k, l$,
$\|\gk\| \le \maxmodelgrad$
so that
\begin{align*}
P_{\feasiblek\left(\mathbb A_1^{(k)}\right)}\left(\xk - \gk\right)
= P_{\feasiblek\left(\mathbb A_1^{(k)}\right) \cap B_2\left(\xk, \maxmodelgrad\right)}\left(\xk - \gk\right) := z_m^{(k)}, \\
P_{\truefeasiblek\left(\mathbb A_3^{(k)}\right)}\left(\xk - \gk\right)
= P_{\truefeasiblek\left(\mathbb A_3^{(k)}\right) \cap B_2\left(\xk, \maxmodelgrad\right)}\left(\xk - \gk\right) := z_l^{(k)}, \\
P_{\feasiblel\left(\mathbb A_2^{(k)}\right)}\left(\xk - \gk\right)
= P_{\feasiblel\left(\mathbb A_2^{(k)}\right) \cap B_2\left(\xk, \maxmodelgrad\right)}\left(\xk - \gk\right) := z_c^{(k)}.
\end{align*}
Let $\activeindicesk$ be defined by \cref{define_activeindicesk}.
By assigning $\delta_r \gets \minangledelta$ and partitioning either
\begin{align*}
\begin{array}{cc}
C  \gets \left[\nabla m^{(k)}_c\left(\xk\right)\right]_{\activeindicesk \cap \mathbb A_1^{(k)}}, &
C' \gets \left[\nabla c\left(\xk\right)\right]_{\activeindicesk \cap \mathbb A_3^{(k)}}, \\
A  \gets \left[\nabla m^{(k)}_c\left(\xk\right)\right]_{([m] \setminus \activeindicesk) \cap \mathbb A_1^{(k)}}, &
A' \gets \left[\nabla c\left(\xk\right)\right]_{([m] \setminus \activeindicesk) \cap \mathbb A_2^{(k)}} \\
d  \gets \left[-m^{(k)}_c\left(\xk\right)\right]_{\activeindicesk \cap \mathbb A_1^{(k)}}, &
d' \gets \left[-c\left(\xk\right)\right]_{\activeindicesk \cap \mathbb A_3^{(k)}}, \\
b  \gets \left[-m^{(k)}_c\left(\xk\right)\right]_{([m] \setminus \activeindicesk) \cap \mathbb A_1^{(k)}}, &
b' \gets \left[-c\left(\xk\right)\right]_{([m] \setminus \activeindicesk) \cap \mathbb A_2^{(k)}} \\
\end{array}
\end{align*}
or
\begin{align*}
\begin{array}{cc}
C  \gets \left[\nabla m^{(k)}_c\left(\xk\right)\right]_{\activeindicesk \cap \mathbb A_1^{(k)}}, &
C' \gets \left[\nabla m^{(l)}\left(\xk\right)\right]_{\activeindicesk \cap \mathbb A_3^{(k)}}, \\
A  \gets \left[\nabla m^{(k)}_c\left(\xk\right)\right]_{([m] \setminus \activeindicesk) \cap \mathbb A_1^{(k)}}, &
A' \gets \left[\nabla m^{(l)}_c\left(\xk\right)\right]_{([m] \setminus \activeindicesk) \cap \mathbb A_2^{(k)}} \\
d  \gets \left[-m^{(k)}_c\left(\xk\right)\right]_{\activeindicesk \cap \mathbb A_1^{(k)}}, &
d' \gets \left[-m^{(l)}\left(\xk\right)\right]_{\activeindicesk \cap \mathbb A_3^{(k)}}, \\
b  \gets \left[-m^{(k)}_c\left(\xk\right)\right]_{([m] \setminus \activeindicesk) \cap \mathbb A_1^{(k)}}, &
b' \gets \left[-m^{(l)}_c\left(\xk\right)\right]_{([m] \setminus \activeindicesk) \cap \mathbb A_2^{(k)}}
\end{array}
\end{align*}
we know that we immediately have $\delta_r \|A_i\| \le b_i$ and $\delta_r \|A_i'\| \le b_i'$.
If $\delta$ is defined by \cref{norm_of_active_constraints_bounded_below},
then for sufficiently large $k, l$, \cref{norm_of_active_constraints_bounded_below} and \cref{i_thought_i_proved_this_already} provide
$\delta_l \le \|A_i\| \le \delta_u$ and $\delta_l \le \|A'_i\| \le \delta_u$
where
$\delta_l \gets \delta$ and $\delta_u \gets \maxmodelgrad$.
Also, sufficiently large $k, l$, \cref{yet_another_bound_on_a_u} provides $u \gets \activedirk$ and $\delta_{\alpha} \gets \minactivealpha$ such that
$Cu \ge \delta_{\alpha} e$ and $C'u \ge \delta_{\alpha} e$.
By \cref{delta_to_zero}, \cref{accuracy_is_satisfied}, and \cref{model_gradients_are_cauchy} we know that there exists $k_1 \in \naturals$ such that
\begin{align*}
\|\gmcik - \nabla c_i\left(\xk\right)\| \le ? \\
\|\gmcik - \gmcil\| \le ? \\
\dk \le ? \\
\end{align*}
so that
...
\end{array}
\end{align*}
We can then assign $r \gets \maxmodelgrad$ and $p \gets \xk$
so that \cref{the_lemma_to_end_all_lemmas} applies, for sufficiently large $k, l$, if
\begin{align*}
\begin{array}{ccccc}
S  &= \bigg\{x \in \Rn \bigg|& A (x - p) \le b ,& C (x - p) \le d ,& \|x - p\| \le r \bigg\}, \\
S' &= \bigg\{x \in \Rn \bigg|& A'(x - p) \le b',& C'(x - p) \le d',& \|x - p\| \le r \bigg\}.
\end{array}
\end{align*}
we have $\convexdistance(S, S') \le \epsilon$ where $\convexdistance$ be defined by \cref{define_a_difference_between_sets}.
The result then follows from \cref{board_tied_to_sphere}.
\end{proof}
