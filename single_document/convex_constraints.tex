...
We will make the following definitions for this section.
\begin{align}
...
\begin{lemma}
There exists $k_0$ and $\huff > 0$ such that if $k, l \ge k_0$, then
\begin{align*}
H\left(A^{(k)}_{\activeprojk \cup \activeprojl}\right) \le \huff
\end{align*}
\end{lemma}
\begin{proof}
\end{proof}
...
\subsubsection{Bound on Huffman Constant, Only Active Constraints}
