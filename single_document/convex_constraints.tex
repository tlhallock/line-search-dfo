...
Here we show how to construct the ellipsoid, and that it satisfies \cref{ellipsoids_exist}.
...
is suitable according to definition \cref{ellipsoids_exist}.
...
Let $\minanglealpha$ and $\minactivegraddelta$ be defined as in \cref{minangleassumption} and define $\thetamink$ as in \cref{define_thetamink}.
...
Suppose \cref{minangleassumption} holds.
...
Let $\activeconstraintsk$ be defined by \cref{define_activeconstraints}.
During iteration $k$, if $\activeconstraintsk = \emptyset$, then the ellipsoid defined by \cref{define_trivial_ellipsek} satisfies \cref{ellipsoids_exist}.
...
If $\activeconstraintsk \ne \emptyset$ and $\dk \le \dacc$, then the ellipsoid defined by \cref{define_ellipsek} satsifies \cref{ellipsoids_exist}.
...
\end{lemma}
\begin{proof}
By \cref{minanglelemmap1}, there exist $\minangledelta' > 0$ and $\minanglealpha'$ such that for each $x \in \feasible$, there is a unit vector $\minangledir'(x)$ such that
\begin{align*}
-\frac {\nabla c_i(x)}{\|\nabla c_i(x)\|}^T \minangledir'(x) \ge \minanglealpha'.
\end{align*}
...
Fix an iterate $k$.
Suppose that $\qk$, $\ck$, and $\sdk$ satisfy \cref{ellipsoids_exist} and 
...
be as generated by \cref{constrained_dfo}: namely,
...
respectively.
\begin{comment}
Replace $\epsilon_{g}$ with $\kappa_g$.
\end{comment}
By \cref{accuracy_is_satisfied}, there exist $\epsilon_{g} > 0$ and $\nu\in\Rn$ with $\|\nu\|\le 1$ such that:
\begin{align}
...
\cref{minangleassumption},
and \cref{mingradassumption} hold.
If $\dk \le \dfeas$, then $\unshiftedellipsoid$ as defined in \cref{define_ellipsek} and \cref{define_trivial_ellipsek} satisfies
...
\begin{align*}
m_f^{(k)}(\xk) - m_f^{(k)}(u_{GC}) \ge \kappa_f\chi_k \min\left\{ \frac{\chi_k}{1+\|\nabla^2 \mfk(\xk)\|}, \frac 1 2 \dk, 1 \right\}.
\end{align*}
\end{lemma}
