...
There exists a $\maxnorm > 0$ such that for each $k \in \naturals$ we have
\begin{align*}
...
Assume \cref{mingradassumption}, \cref{bounded_hessians_assumption} and the assumptions for \cref{bounded_gradients_lemma} and \cref{accuracy_is_satisfied} hold.
Define
\begin{align}
...
We have that $\huff(A_k) \le M_{\Gamma_0} \forall k$
\color{red}
...
M' = 4M_{\Gamma_0}\left(1 + \maxnorm + 2\huff(A^{(k)})\left(1 + \maxnorm\right)\right)
\end{align}
...
\le \frac{1}{2\minactivegrad^2} \left(L_g\left\|\xk - x^{(l)}\right\| + 4\kappa_{g}\dk^2\right)\left(\maxgrad + \kappa_g\dk^2\right) \le \epsilon_1.
\end{align*}
...
\le \frac{2}{2\minactivegrad^2} \max\left\{\maxgrad, c_i(\xk), c_i(x^{(l))}\right\}\max\left\{\lipgrad\left\|\xk - x^{(l)}\right\| + 4\kappa_{g}\dk^2, \lipgrad \|\xk - x^{(l)}\| + \maxhessian\|\xk - x^{(l)}\|^2\right\}
\le \epsilon_1.
\end{align*}
...
=   2\left\|\xk - x^{(l_k)}\right\| + \left\|\gk - \gradf(\xk) + \gradf(\xk) - \gradf(x^{(l_k)}) + \gradf(x^{(l_k)}) - g^{(l_k)}\right\| \\
\le 2\left\|\xk - x^{(l_k)}\right\| + \left\|\gk - \gradf(\xk)\right\| + \left\|\gradf(\xk) - \gradf(x^{(l_k)})\right\| + \left\|\gradf(x^{(l_k)}) - g^{(l_k)}\right\|.
\end{align*}
