...
\sampletrk = \unshiftedellipsoid = \left\{x \in \rn | f_e\left(\frac 1 {2\gamma} \dk, \frac 1 {2\gamma} \dk, \beta^{\star}; R(x - \xk)\right) \le 0\right\}. \label{def_ellipse_k}
\end{align}
\cref{ellsoid_is_feasible_theorem} tells us that this ellipsoid satisfies \cref{ellipsoids_exist}.
...
\begin{align}
\unshiftedellipsoid \subseteq \fcki \cap \tr
\end{align}
where
\begin{align}
...
Note that the criteria is satisfied for $f$ by \cref{lipschitz_hessian}, and for the constraints by \cref{bounded_constraint_hessians}.
...
Also, by \cref{def_f} we know that $-s^T \hgik \ge \beta \dk^{p_{\beta}}$ where $\|s\| = 1$.
By our assumption \cref{delta_is_small_enough}, \cref{def_p_beta}, \cref{z_is_active} we know
\begin{align*}
...
\end{align}
and assume that $\dk \le \Delta_{\beta}$, so that
\begin{align}
...
$\theta^{\text{min}}_k \ge \minanglealpha \Longrightarrow \sqrt{1 - \left(\minanglealpha\right)^2} \ge \sqrt{1 - \left(\theta^{\text{min}}_k\right)^2}$
so that
\begin{align*}
\bsk 
...
\Longrightarrow (x - \epsilon e_1)^T\bigg(\begin{bmatrix}
1 & \boldsymbol0^T \\
\boldsymbol 0 & \frac{\theta^2}{1 - \theta^2} \boldsymbol I \\
\end{bmatrix}\bigg)(x - \epsilon e_1) \le \frac 1 2 \delta^2 \\
\Longrightarrow (e_1^Txe_1 + (x - e_1^Txe_1) - \epsilon e_1)^T\bigg(\begin{bmatrix}
1 & \boldsymbol0^T \\
\boldsymbol 0 & \frac{\theta^2}{1 - \theta^2} \boldsymbol I \\
\end{bmatrix}\bigg)(e_1^Txe_1 + (x - e_1^Txe_1) - \epsilon e_1) \le \frac 1 2 \delta^2 \\
\Longrightarrow 
...
\Longrightarrow 
\left[\left(ts^Te_1 - \delta\right) e_1 + t\left(s - \left(s^Te_1\right) e_1\right)\right]^T\bigg(\begin{bmatrix}
1 & \boldsymbol0^T \\
\boldsymbol 0 & \frac{\theta^2}{1 - \theta^2} \boldsymbol I \\
\end{bmatrix}\bigg)\left[\left(ts^Te_1 - \delta\right) e_1 + t\left(s - \left(s^Te_1\right) e_1\right)\right] \le \frac 1 2 \delta^2 \\
\Longrightarrow
...
\begin{align*}
\qk = I, \quad \ck = \xk, \quad \delta_k = \sqrt{2} \dk.
\end{align*}
...
Finally, $(\xk - \xk)^T\qk(\xk - \xk) = 0 \le \delta_k^2$.
\end{proof}

\begin{lemma}
\label{ellsoid_is_feasible_theorem}
Assume that

hold, and suppose that $\activeconstraintsk \ne \emptyset$.
Let
\begin{align}
\dk \le \min\left\{
1,
\Delta_{\beta}
\right\}\label{accuracy_small_delta}\\
\beta^{\star} = \max\left\{\frac 1 2, \bsk\right\} \\
\gamma = 1 + \frac 1 {\sqrt 2}
\end{align}
where $\Delta_{\beta}$ is defined in \cref{define_boundedbeta_deltasmall}, and $R$ be defined as in \cref{def_rotation}.
Then, \cref{ellipsoids_exist} is satisfied by letting
\begin{align*}
\qk = R^T \begin{bmatrix}
1 & \boldsymbol0^T \\
\boldsymbol 0 & \frac{\beta^{\star}}{1 - \beta^{\star}} \boldsymbol I \\
\end{bmatrix} R, \quad
\ck = \xk  + \frac 1 {2\gamma} \dk\huk, \quad
\delta_k = \frac 1 {2\gamma} \dk.
\end{align*}
% as in \cref{def_ellipse_k}.
\end{lemma}
\begin{proof}
...
= \left(x - \left(\xk + \frac 1 2 \dk \huk\right)\right)^TR^T\bigg(\begin{bmatrix}
1 & \boldsymbol0^T \\
\boldsymbol 0 & \frac{\left(\beta^{\star}\right)^2}{1 - \left(\beta^{\star}\right)^2} \boldsymbol I \\
\end{bmatrix}\bigg) R\left(x - \left(\xk + \frac 1 2 \dk \huk\right)\right) - \frac 1 2 \delta^2 \\
= \left(R(x-\xk) - \frac 1 2 \dk e_1\right)^T\bigg(\begin{bmatrix}
1 & \boldsymbol0^T \\
\boldsymbol 0 & \frac{\left(\beta^{\star}\right)^2}{1 - \left(\beta^{\star}\right)^2} \boldsymbol I \\
\end{bmatrix}\bigg) \left(R(x-\xk) - \frac 1 2 \dk e_1\right) - \frac 1 2 \delta^2 \\
= f_e\left(\frac 1 2 \dk, \delta, \beta^{\star}; R\left(x - \xk\right)\right).
\end{align*}
In particular, letting $\delta = \delta_k$ and $\delta = \sqrt{2}\delta_k$, we see that
\begin{align*}
