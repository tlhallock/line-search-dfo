...
-1 \\
\end{pmatrix}, \quad
\btrk = \atr \xk + \dk e_{\pm}. \label{define_atr}
\end{align}
In row form, this is
\begin{align*}
...
\begin{lemma}
Let $\atr$ be defined by \cref{define_atr}.
Then $H(\atr) = 1$.
\end{lemma}
\begin{proof}
Let $e_{\pm}$ be defined by \cref{define_atr}.
Let $p, y \in \Rn$ and $r > 0$ be arbitrary, and define
\begin{align*}
\begin{array}{cccccc}
z =   & \argmin_{x \in \Rn} & \|x - y\| 						&  = & \argmin_{x \in \Rn} & \|x - y\| \\
      & \textrm{s.t.}       & x \in B_{\infty}\left(p, r\right)	&    & \textrm{s.t.}x	   & \atr \le p + re_{\pm} 
\end{array}.
\end{align*}
% so that
% \begin{align*}
...
% \end{align*}
Let $v = \atr y - re_{\pm}$, then for any $i \in [m]$ we have
$v_{2i} = y_i - r$ and $v_{2i + 1} = -y + r$.
...
\begin{lemma}
Suppose that $a, v \in \Rn$, $b, r \in \reals$, and $i \in [m]$ are given such that
\begin{align*}
a^Tv > b \\
0 \le b \\
r \ge 0 \\
r \le v_i \\
-2r \le v_j \le 2r \quad \forall j \in [m]
\end{align*}
Then $a_i > ?$.
\end{lemma}
\begin{proof}
Because $a^Tv > b \ge 0$, we know that
\begin{align*}
a_i v_i \ge - \sum_{\substack{j=1 \\ j\ne i}}^n a_j v_j.
\end{align*}
Because $r < v_i < 2r$,
\begin{align*}
\frac 1 r > \frac 1 {v_i} > \frac 1 {2r},
\end{align*}
so
\begin{align*}
a_i\ge - \frac 1 {2r} \sum_{\substack{j=1 \\ j\ne i}}^n a_j v_j.
\end{align*}
Because $-2r \le v_j \le 2r$, 
\begin{align*}
a_i\ge - \frac 1 {2r} \sum_{\substack{j=1 \\ j\ne i}}^n a_j 2r \frac{a_j}{|a_j|} = -\sum_{\substack{j=1 \\ j\ne i}}^n |a_j|.
\end{align*}

Now, because $\|a\| = 1$,
\begin{align*}
a_i \ge -\sum_{\substack{j=1 \\ j\ne i}}^n |a_j| 
\ge \min_{\|u\|=1} -\sum_{\substack{j=1 \\ j\ne i}}^n |u_j| 
= -\max_{\|u\|=1} \sum_{\substack{j=1 \\ j\ne i}}^n |u_j| 
= -\left(n-1\right)\frac{1}{\sqrt{n-1}}
\end{align*}
% = -\max_{\|u\|=1} \left[-|u_i| + \sum_{j=1}^n |u_j| \right]
% = -\max_{\|u\|=1} \left[|u_i| + \|u\|_1 \right]
% By symmetry,
% \begin{align*}
% a_i \ge -\max_{\|u\|=1} \left[u^Te_i + \|u\|_1 \right]
% \end{align*}
\begin{align*}
\end{align*}
...
Let $A_1$ be an $m \times n$ matrix, $p \in \Rn$, and $b_1 \in \Rm$  be given 
such that $A_1p \le b_1$.
Define 
\begin{align*}
A_2 = \begin{pmatrix} A_1 \\ \atr \end{pmatrix}.
\end{align*}
Then $H(A_2) = \max\left\{H\left(A_1\right), \left(1 + \sqrt{n}\right), ?\right\}$.
\end{lemma}
\begin{proof}
...
Define
\begin{align*}
z_0 = \argmin_{\substack{x \in B_{\infty}(p, r), \\ A_1 x \le b_1} } \|y - x\|, \quad
z_1 = \argmin_{A_1 x \le b_1} \|y - x\|, \quad \textrm{and} \quad
z_2 = \argmin_{x \in B_{\infty}(p, r)} \|y - x\|.
\end{align*}
% and
% \begin{align*}
% \begin{array}{cc}
...
Then $\|y - z_0\| \le H(A_1) \left\|\left(A_1y - b_1\right)_+\right\|_{\infty} \le H\left(A_1\right) \left\|\left(A_2y - b_2\right)_+\right\|_{\infty}$.
\paragraph{Case 2}
Suppose that $y \not \in B_{\infty}(p, 2r)$.

Then $\left\|\left(A_2y - b_2\right)_+\right\|_{\infty} \ge r$,
so that $\|y - z_0\| 
\le \|y - z_2\| + \|z_2 - z_0\| 
=  \left\|\left(\atr y - p + re_{\pm} \right)_+\right\|_{\infty} + \|x_0 - z_2\| \\
\le \left\|\left(A_2y - b_2\right)_+\right\|_{\infty} + r\sqrt{n}
\le \left(1  + \sqrt{n} \right) \left\|\left(A_2y - b_2\right)_+\right\|_{\infty}
$

\paragraph{Case 3}
Suppose that $y \in B_{\infty}(p, 2r)$.
...
\end{align}
Finally, the equivalent definitions for the linearizations of the constraints are
\begin{align}
...
\begin{array}{ccc}
\argmin_{s \in \Rn} & \left\|s\right\| & \\
\textrm{s.t.} & c_i\left(\xk\right) + \nabla c_i\left(\xk\right)^T\left(s - \gk\right) & \forall i \in \activeprojkk \cup \trueactiveprojk
\end{array} \\
=
\xk - \gk + \left(\begin{array}{ccc}
\argmin_{x \in \Rn} & \left\|x - \left(\xk - \gk\right)\right\| & \\
\textrm{s.t.} & c_i \left(\xl\right) + \nabla c_i\left(\xk\right) ^T\left(x - \xk \right) & \forall i \in \activeprojkk \cup \trueactiveprojk
\end{array}\right) \\
=
\xk - \gk + \left(\begin{array}{ccc}
\argmin_{x \in \Rn} & \left\|x - \left(\xk - \gk\right)\right\| & \\
\textrm{s.t.} & c_i \left(\xl\right) + \nabla c_i\left(\xk\right) ^T\left(x - \xk \right) & \forall i \in [m]
\end{array}\right) \\
=
...
Let $A_1, b_1, A_2, b_2, P_1, P_2$ be defined by \cref{define_the_as} and \cref{define_the_ps}.
...
\end{align*}
\end{proof}
Using \cref{delta_to_zero}, \cref{accuracy_is_satisfied}, \cref{bouneded_projection_model_to_constraints}, 
