...
%
%
% Make the uses of [m] be 1 \le i \le m before table of notation....
% Add discussion of convergence of iterates

% consolidate kappa_ef and kappa_f


% Replace the eta/gamma suff
%
% remove latex warnings
% simplify the algorithm                                                                                                                                            XXXXXXX (just remove that section)
% make a comment about the decision to construct the next ellipsoid (proximity vs large enough)                         XXXXXXX
...
% make the huffman assumption only require nearly active constraints
% 
% 
% 
% 
...
% \left\|A_i\right\| = \left\| A'_i \right\| = 1 \quad \forall i \in [m], \\
\begin{align*}
Ap \le b,
\quad 
A'p \le b', \\
...
J = \left\{i \in [m] \; \bigg | \; b_i - A_ip \le \|A_i\|r  \right\} \\
p_J = \argmin_{A_Jz\le b_J} \|y - z\|,
\quad
p'_J = \argmin_{A'_Jz\le b'_J} \|y - z\| \\
z = \argmin_{Az\le b} \|y - z\|,
\quad
z' = \argmin_{A'z\le b'} \|y - z\| \\
...
\begin{itemize}
\item if $k \ge k_0$, then if $i \in \mathbb A_1 \cup \mathbb A_3$
$\left\|\gmcik\right\| \ge \delta$ and 
$\left\|\nabla c_i\left(\xk\right) \right\| \ge \delta$.
\item if $k, l \ge k_0$ and $(k, l) \in S$, then if $i \in \mathbb A_1 \cup \mathbb A_2$
...
\paragraph{Case 1}
If $-c_i(\xk) \le \mingradepsilon$, then $\| \nabla c_i(\xk) \| \ge \mingrad \ge 3\delta$.
\paragraph{Case 2}
Suppose that $-c_i(\xk) > \mingradepsilon$ and $i \in \mathbb A_1$.
\begin{align*}
...
\end{align*}
By \cref{hmoydihtp_eqn1}, we have $\| \nabla c_i(\xk) \| \ge 3\delta$

\paragraph{Case 3}
Suppose that $-c_i(\xk) > \mingradepsilon$ and $i \in \mathbb A_2$.
\paragraph{Case 4}
Suppose that $-c_i(\xk) > \mingradepsilon$ and $i \in \mathbb A_3$.

Throughout the cases, we have established that $\left\| \nabla c_i(\xk) \right\| \ge 3 \delta$.
