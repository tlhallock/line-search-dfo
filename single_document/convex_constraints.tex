...
For some fixed $k$, suppose that $\Amk$, $\Alk$, $\Ack$
are defined by
...
\begin{itemize}
\item if $k \ge k_0$, then if $i \in \Amk \cup \Ack$, we have
\begin{align*}
\left\|\gmcik\right\| \ge \delta \quad \textrm{and} \quad
\left\|\nabla c_i\left(\xk\right) \right\| \ge \delta.
\end{align*}
\item if $k, l \ge k_0$ and $(k, l) \in S$, then if $i \in \Amk \cup \Alk$, we have
\begin{align*}
...
\paragraph{Case 2}
Suppose that $-c_i\left(\xl\right) > \mingradepsilon$ and $i \in \Amk$.
\begin{align*}
...
\paragraph{Case 3}
Suppose that $-c_i\left(\xl\right) > \mingradepsilon$ and $i \in \Alk$.
\begin{align*}
...
\paragraph{Case 4}
Suppose that $-c_i\left(\xl\right) > \mingradepsilon$ and $i \in \Ack$.
\color{red}
...
Notice that
\begin{align*}
\zmk =& P_{\feasiblek\left(\Amk\right)}\left(\xk - \gk\right) \\
\zlk =& P_{\feasiblel\left(\Alk\right)}\left(\xk - \gk\right) \\
\zck =& P_{\truefeasiblek\left(\Ack\right)}\left(\xk - \gk\right)
\end{align*}
Further, notice that by \cref{i_thought_i_proved_this_already} and \cref{delta_to_zero}, for sufficiently large $k, l$,
$\|\gk\| \le \maxmodelgrad$
so
\begin{align*}
\zmk =& P_{\feasiblek\left(\Amk\right) \cap B_2\left(\xk, \maxmodelgrad\right)}\left(\xk - \gk\right) \\
\zlk =& P_{\feasiblel\left(\Alk\right) \cap B_2\left(\xk, \maxmodelgrad\right)}\left(\xk - \gk\right) \\
\zck =& P_{\truefeasiblek\left(\Ack\right) \cap B_2\left(\xk, \maxmodelgrad\right)}\left(\xk - \gk\right)
\end{align*}
Let $\activeindicesk$ be defined by \cref{define_activeindicesk}.
