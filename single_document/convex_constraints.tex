...
\forall S \subseteq [m], \feasiblek(S) =  \left\{ x \in \Rn \bigg| \mcik\left(\xk\right) + \gmcik^T \left(x - \xk\right) \le 0 \forall i \in S \right\} \\
\projkl(S) = \argmin_{x \in \feasiblel(S)} \left\|\xk - \gk\right\| \\
\activeprojk = \left\{ i \in [m] \bigg| \mcik\left(\xk\right) + \gmcik^T \left(\projkk([m]) - \xk\right) = 0 \right\} \\
A^{(k)} = \nabla m_c\left(\xk\right).
\end{align}


\begin{assumption}
\label{huffman_assumption}
There exist $\epsilon, \alpha > 0$ such that
for each $x \in \Omega$ and unit vector $y \in \Rn$,
there exists a unit vector $u(x, y) \in \Rn$ such that
if $1 \le i \le m$ is such that 
\begin{align*}
y^T \nabla c_i \left(x\right) \ge - \epsilon
\end{align*}
then
\begin{align*}
u(x, y)^T \nabla c_i \left(x\right) \ge \alpha.
\end{align*}
\end{assumption}


\begin{lemma}
Suppose that $i \in \activeprojk \cup \activeprojl$ and $k, l \ge k_0$.
There exists a unit vector $ u \in \Rn $ with
\begin{align*}
 u^T \gmcik \ge \frac 1 2 \alpha
\end{align*}
%  \quad \textrm{and} \quad
...
If $i \in \activeprojl$, then
\begin{align*}
-\gl^T \gmcil \ge 0 \\
\Longrightarrow -\gk^T \gmcik \ge - \frac 1 2 \epsilon
\end{align*}
which is a case of the following.

If $i \in \activeprojk$, then
\begin{align*}
-\gk^T \gmcik \ge 0 \ge - \frac 1 2 \epsilon \\
\Longrightarrow -\gk^T \nabla c_i \left(\xk\right) \ge -\epsilon \\
\Longrightarrow u\left(\xk, -\gk\right)^T \nabla c_i \left(\xk\right) \ge \alpha \\
