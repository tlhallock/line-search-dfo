...
\activeprojk = \left\{ i \in [m] \bigg| \mcik\left(\xk\right) + \gmcik^T \left(\projkk([m]) - \xk\right) = 0 \right\} \label{sha_proj_indices} \\
...
Let $\activeprojk$ and $\huffalpha$ be defined by \cref{sha_proj_indices} and \cref{huffman_assumption}.
Assume that \cref{huffman_assumption}, \cref{lipschitz_gradients_assumption} and the assumptions for
\cref{model_gradients_are_cauchy}, \cref{delta_to_zero}, \cref{bounded_gradients_lemma}, \cref{i_thought_i_proved_this_already}, and \cref{accuracy_is_satisfied} hold.
There exists $k_0 \in \naturals$ such that for any $k, l \ge k_0$ 
there is a unit vector $ \huffdirk \in \Rn $
such for any $i \in \activeprojk \cup \activeprojl$,
\begin{align*}
...
\end{lemma}
\begin{proof}
Let $i \in \activeprojk \cup \activeprojl$.
Let 
$\huffeps$, $\maxgrad$, and $\maxmodelgrad$
be defined by \cref{huffman_assumption}, \cref{bounded_gradients_lemma}, and \cref{i_thought_i_proved_this_already}
respectively.
Define
\begin{align}
M = \max\left\{\maxgrad, \maxmodelgrad\right\} \label{til_define_m} \\
\epsilon = \frac 1 {4M} \huffeps. \label{til_define_eps}
\end{align}

From \cref{accuracy_is_satisfied}, there exists $\kappa_g$ and $k_1$ such that if $k, l \ge k_1$, then
\begin{align}
...
By \cref{lipschitz_gradients_assumption}, we know that there exists $\lipgrad$ such that
\begin{align}
\label{til_lip}
\left\|\nabla c_i\left(\xk\right) - \nabla c_i\left(\xl\right)\right\| \le \lipgrad \left\|\xk - \xl \right\| \\
\quad \textrm{and} \quad
\left\|\nabla f\left(\xk\right) - \nabla f\left(\xl\right)\right\| \le \lipgrad \left\|\xk - \xl \right\|.
\end{align}
 
\cref{bounded_gradients_lemma}, \cref{i_thought_i_proved_this_already}, and \cref{til_define_m} tell us
\begin{align}
...
\quad \textrm{and} \quad
\left\|\nabla c_i \left(\xk\right)\right\| \le M.
\end{align}
...
By \cref{delta_to_zero}, we know that there exists $k_2 \in \naturals$ such that if $k, l \ge k_2$, then
\begin{align}
\label{til_bound_dk}
\left\|\xk - \xl \right\| \le \frac{\epsilon}{2\lipgrad}
\quad \textrm{and} \quad
\dk \le \min\left\{
\sqrt{\frac{\epsilon}{4\kappa_g}},
\sqrt{\frac{\huffalpha}{2\kappa_g}}
\right\} \quad \textrm{and} \quad
\dl \le \sqrt{\frac{\epsilon}{4\kappa_g}}.
\end{align}

By the triangle inequality, \cref{til_acc_obj}, \cref{til_lip}, and \cref{til_bound_dk} we have
\begin{align}
\left\| \gk - \gl \right\|
...
Likewise, \cref{til_acc_const}, \cref{til_lip}, and \cref{til_bound_dk} provide
\begin{align}
...
\begin{align}
\left\|\gk^T \gmcik - \gl^T \gmcil\right\| \le 2M\epsilon \le \frac 1 2 \huffeps. \label{til_case_one}
\end{align}
...
If $i \in \activeprojl$, then $-\gl^T \gmcil \ge 0$ so that \cref{til_case_one} implies $-\gk^T \gmcik \ge - \frac 1 2 \huffeps$.
On the other hand, if $i \in \activeprojk$, $-\gk^T \gmcik \ge 0 \ge - \frac 1 2 \huffeps$.
Either way, we have
\begin{align*}
...
\le M\kappa_g\dk^2 
\le M\epsilon \le \frac 1 2 \huffeps
\end{align*}
so that
\begin{align*}
...
\left(\huffdirk\right)^T \gmcik \ge \frac 1 2 \huffalpha.
\end{align*}
Thus, we only need to let $k_0 = \min\left\{k_1, k_2\right\}$.
% 
% 
% \paragraph{Case 2}
% If $i \in \activeprojk$, then
% \begin{align*}
% \Longrightarrow  \\
...
Let $A^{(k)}$ and $\activeprojk$ be defined by \cref{sha_jacobian} and \cref{sha_proj_indices} respectively.
Suppose that the assumptions for \cref{bp_combining_several} hold.
There exists $k_0 \in \naturals$ and $\huff > 0$, such that if $k, l \ge k_0$, then 
\begin{align*}
H\left(A^{(k)}_{\activeprojl \cup \activeprojk}\right) \le \huff.
\end{align*}
\end{lemma}
\begin{proof}
...
That is,
\begin{align*}
A^{(k)}_{\activeprojl \cup \activeprojk} u_f^{(k,l)} \ge \frac 1 2 \huffalpha e.
\end{align*}

Let $J \subseteq \activeprojl \cup \activeprojk$ with $|J| \ne 0$ be arbitrary,
and suppose that $v_J \in \mathbb R^J_+$ with $\|v_J\| = 1$.
Then, Cauchy-Schwarz implies
 \begin{align*}
 \left\|\left(A^{(k)}_J\right)^Tv\right\| = \left\|\left(A^{(k)}_J\right)^Tv\right\| \left\|\huffdirk\right\| \ge
\left(\left(A^{(k)}_J\right)^Tv\right)^T\huffdirk
= v^T\left(A^{(k)}_J\right) \huffdirk
...
so that 
\begin{align*}
\left[{\|A_J^Tv\|}\right]^{-1} \le \frac 2 {\huffalpha}.
\end{align*}
...
\begin{align*}
H\left(A^{(k)}_{\activeprojl \cup \activeprojk}\right) = \max_{J \in \mathcal S\left(A^{(k)}_{\activeprojl \cup \activeprojk}\right)} \left[\min_{v \in \mathbb R^J_+, \|v\| = 1}  \left\|A_J^Tv\right\| \right]^{-1} \le \frac 2 {\huffalpha} = \huff.
\end{align*}
\end{proof}
...
\begin{proof}
\begin{align*}
\left\|\projkl([m]) - \projkk([m]) \right\| = \left\|\projkl\left(\activeprojk \cup \activeprojl\right) - \projkk\left(\activeprojk \cup \activeprojl\right) \right\|
\end{align*}
For $k \ge k_0$, 
\begin{align*}
\left\|A^{(k)}_{\activeprojk \cup \activeprojl} - A^{(l)}_{\activeprojk \cup \activeprojl}\right\| \le \epsilon_1 \\
\textrm{and} \quad \left\|c\left(\xk\right)_{\activeprojk \cup \activeprojl} - c\left(\xl\right)_{\activeprojk \cup \activeprojl}\right\| \le \epsilon_1 \\
\textrm{and} \quad H\left(A^{(k)}_{\activeprojk \cup \activeprojl}\right) \le \epsilon_1 \\
\textrm{and} \quad H\left(A^{(l)}_{\activeprojk \cup \activeprojl}\right) \le \epsilon_1 \\
\Longrightarrow \left\|\projkl\left(\activeprojk \cup \activeprojl\right) - \projkk\left(\activeprojk \cup \activeprojl\right) \right\| \le  \epsilon
\end{align*}
\end{proof}
\color{black}
...
Let
$A_1$, $A_2$, $h$, $M$, $\huff$
be defined by \cref{bap_define_the_polyhedra}.

Suppose that $\epsilon\le \min\left\{\frac{\min_i h_i}{1 + M},\left(2 H\left(A_1\right)\right)^{-1}\right\}$.
...
\label{2_2}
There exists a $\epsilon_{\textrm{huff}}\left(h, \huff\left(A_1\right), \huff\left(A_2\right), M\right) > 0$.
such that if $\epsilon \le $.
...
We define
\begin{align*}
\epsilon_{\textrm{huff}}\left(h, \huff\left(A_1\right), \huff\left(A_2\right), M\right)
=
