...
\image(A) = \left\{Ax \in \Rm \;|\; x \in \Rn \right\}
\end{align*}
...
\begin{align*}
H\left(\jackkl\right) = \max_{J \in \mathcal S_h\left(\jackkl\right)} \left[\min_{v \in \mathbb R^J_+, \|v\| = 1}  \left\|\left(\jackj\right)^Tv\right\| \right]^{-1} \le \frac 2 {\huffalpha} = \huff.
\end{align*}
\end{proof}
...
\begin{align*}
\mcik\left(\xk\right) + \gmcik^T y \le \gmcik^T\gk
\end{align*}

Then there exists a $y'$ such that
\begin{align*}
\mcil\left(\xl\right) + \gmcik^Ty' \le \gmcil^T \left[\xl - \xk + \gk \right] + \left[\gmcik - \gmcil\right]^T y
\end{align*}
\end{lemma}
\begin{proof}
...
hold.
Then, for each $i \in [m]$, the sequence $\left\{\gmcik\right\}_{i=1}^{\infty}$ is Cauchy.
\end{lemma}
\begin{proof}
...
\begin{array}{ccc}
\argmin_{x \in \Rn} & \left\|x - \left(\xk - \gk\right)\right\| & \\
\textrm{s.t.} & \mcik \left(\xk\right) + \gmcik ^T\left(x - \xk\right) & \forall i \in [m]
\end{array} \\
=
...
\begin{array}{ccc}
\argmin_{x \in \Rn} & \left\|x - \left(\xk - \gk\right)\right\| & \\
\textrm{s.t.} & \mcil \left(\xl\right) + \gmcil ^T\left(x - \xl\right) & \forall i \in [m]
\end{array} \\
=
...
Take $ \tilde \Delta = \min \{\frac{\epsilon}{\maxhessian}, \frac{(1 - \gammasm)c}{\epsilon}, \left(\frac 1 {\kappa_{\chi}}  \epsilon \right)^{\frac 1 {p_{\Delta}}}, 1\}$
...
Suppose that \cref{the_contradiction} holds.
...
\end{align}
Then the sequence $S_{lb}$ satisfies \cref{criteria_from_contradiction}.
\end{lemma}
\begin{proof}

Let $S$ be defined by \cref{define_S}.
...
\begin{align*}
f(x^{(i)}) - f(x^{(i+1)}) \ge \gammabi\big ( \mfk(x^{(i)}) - \mfk(x^{(i)} + s^{(i)}) \big ) \ge \gammabi \kappa_f \chi_i \min\{\frac{\chi_{i}}{\maxhessian}, \Delta_i, 1\} 
\end{align*}
...
It follows that
\begin{align*}
f(x^{(i)}) - f(x^{(i+1)}) > \frac{\gammabi \kappa_f \epsilon \Delta_i}{4}
\end{align*}

and hence
\begin{align*}
\Delta_i < \frac{4}{\gammabi \kappa_f \epsilon} \big ( f(x^{(i)}) - f(x^{(i+1)})\big ).
\end{align*}
...
\begin{align}
\|x^{(i)} - x^{(l_k)}\| < \frac{4}{\gammabi \kappa_f \epsilon} \big ( f(x^{(i)}) - f(x^{(i+1)})\big ).
\end{align}
...
Further suppose that $\gammabi > 0$.
Then $\lim_{k\to\infty}\chik=0$.
\end{lemma}


\begin{proof}
Suppose for a contradiction that for some $\epsilon > 0$ the set  is infinite.
By \cref{delta_to_zero}, there exists a $k_0 \in \naturals$ such that for all $k \ge k_0$,

\begin{align*}
\dk \le \min\left\{\frac{\epsilon}{\maxhessian}, \frac{(1-\gammasm)\epsilon}{c}, \left(\frac 1 {\kappa_{\chi}}  \epsilon \right)^{\frac 1 {p_{\Delta}}}, 1\right\}.
\end{align*}
