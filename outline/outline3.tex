...
Derivatives cannot be calculated directly, and function evaluations must be conserved as they are computationally expensive.
Derivatives have historically played a key role in development of optimization algorithms.
For example, they frequently appear in optimality conditions and search directions, such as the KKT, Wolfe, and Armijo conditions.
%Thus, one ___ of DFO is form alternative methods ....
...
For example, we can look typical constrained program within DFO is given by:
...
For example, there is a host of libraries for Box Constrained DFO (BCDFO) of the form $b_{L} \le x \le b_{U}$ for some $b_{L} < b_{U}$ SPBOX, PSwarm, all NLopt algorithms except COBYLA (BOBYQA, NEWUOA, PRAXIS, Sbplx).

However, the problem becomes much more difficult when no derivative information of $c$ is known: for example, if they are also output from a simulation used to evaluate the objective. This is what we turn our attention to.
...
functions that approximate the original functions in a trust region.
Locally minimizing the model functions becomes much easier as their derivatives are easily calculated.
...
When the constraints are partially-quantifiable, the goal is modify the trust region to avoid evaluating the simulation function outside of the feasible region.
