...
% Remove the outer bullet points
...
\begin{frame}{Geometry of the Sample Set}
	\begin{itemize}
		...
		\item<1, 2, 3> When the points are not ``well poised", the constructed model can be inaccurate \\
		\only<2>{
			\includegraphics[width=120px]{images/poised_good.png} \includegraphics[width=120px]{images/poised_bad.png}
		}
		\item<3> Constructing poised sets over ellipses is well known \cite{introduction_book}
...
%			Nearly active constraints
		...
		\end{center}
% 		\item[\textbf{Step 3}] Compute an ellipsoid within this cone
	\end{itemize}
\end{frame}
...
% 		show how to choose well-poised sample points from an ellipsoidal region
% 		\begin{itemize}
% 			\item[Step 1] \textbf{Construct a model for the current point} \\
...
% 				\color{blue} by searching over $B_{\infty}\left(\xk, \Delta_k\right) \cap F$\color{black}
% 		\end{itemize}
