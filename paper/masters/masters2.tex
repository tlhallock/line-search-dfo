...
Previously, we have shown convergence of our algorithm for linear constraints.
The algorithm follows the same general strategy as the one discussed here; however, certain aspects are simplified.
With linear constraints, the method we use for creating a buffer around the constraints is not needed, and it is simple to construct a feasible ellipsoid.
Also, the projection onto the feasible region is known and easily computed, so that the convergence results are simpler.
This is discussed further in \cref{review_of_linear}.
...
% With spherical trust regions, it is common practive to first shift the sample set $Y$ by subtracting the current iterate and dividing by the trust region radius:
% \begin{align}
...
\begin{algorithm}[H]
    ...
    \begin{itemize}
        ...
        \item[\textbf{Step 2}] \textbf{(Check threshold)} \begin{itemize}
                ...
                \item[] $Y \gets Y \setminus \{ y^i \} \cup \{\hat y\}$
            \end{itemize}
        ...
        \item[\textbf{Step 3}] \textbf{(LU)} \begin{itemize}
            ...
            \end{itemize}
            If $i = p$ then \textbf{Return} $Y$, otherwise Set $i \gets i+1$ and go to Step 1
    \end{itemize}
\end{algorithm}


At the end of this algorithm, the points in $Y$ will be $\Lambda$-poised for some $\Lambda$ depending on the constant $\ximin$.
This algorithm can also be used to create a poised set over an ellipsoidal shape, this is discussed further in \cref{poised_ellipsoid}.
...
We can compute $\chik$ as
\begin{align}
\label{critical}
t^{(k)} = \min_{t \in \feasiblek} \|\iteratek - \nabla \modelk(\iteratek) - t\|^2, 
\quad \chik = \|\iteratek - t^{(k)} \|^2.
\end{align}
...
\begin{algorithm}[H]
    ...
    \begin{itemize}
        \item[\textbf{}] \textbf{(Test for improvement)} \\
...
\section{Review of Linear Constraints}
\label{review_of_linear}

\subsubsection{Trust Regions}
While creating the algorithm for linear constraints, we introduced two separate trust regions.
...
While creating the algorithm for linear constraints, we discussed several different alternatives for some choices made here.
We discussed an ellipsoidal inner trust region versus a polyhedral inner trust region, as well as using the outer trust region within the trust region subproblem.


The classical methods for ensuring good geometry require an optimization call to the model functions over a sphere.
This is no longer possible for a polyhedral trust region approach.
...
\subsection{Maximum Volume Ellipsoid}

We chose to use an ellipsoidal trust region for the inner trust region and use the feasible region intersect the outer trust region for the trust region subproblem.
However, we also discussed several ways to define the inner ellipsoid.
Most of these methods invovled maximizing the volume of an ellipsoid within the polyhedron given from the linearized constraints and outer trust region.
We review this ellipsoid here.
In the previous paper, we also performed a search for the center of the ellipsoid.
...
\subsection{Towards Convex Constraints}
If we tried to define this ellipsoid using the linearization of the constraints for convex constraints, we would no longer know that the ellipsoid is feasible.
In order to handle this, we created a buffer around the constraints that approaches the linearization as $\dk \to 0$, but also ensures that any ellipsoid constructed within the buffered region is feasible.

These buffered region is constructed as cones pointed outward from the linearized constraints.
In particular, we first choose two constants

\begin{align}
\alpha, \beta \in (0, 1) \label{def_alpha_beta}
\end{align}

and find the modelled zero along the negative gradient:
\begin{align}
...
However, we then choose the vertex $\wik$ of the buffering cone to be a short distance back towards the current iterate:
\begin{align}
\wik = \xk + \left(1 - \alpha\dk^{\frac 1 2}\right)\left(\zik - \xk\right) \label{def_w}
\end{align}

We then construct one buffering cone for each constraint:
...
\begin{figure}[h]
    \centering
	\includegraphics[width=150px]{presentations/images/explanation_1.png}
	\includegraphics[width=150px]{presentations/images/explanation_2.png}
	\includegraphics[width=150px]{presentations/images/explanation_3.png}
    \caption{
		On the left is the linearization of a constraint (blue), which may not be within the true constraint (black)
		In the middle, we have created a buffering cone (red) with a vertex along a ray from the current iterate (star) torward the constraint
		On the right, we see that as the trust region gets smaller, this buffering cone approaches the true constraint.
	}
    \label{pvip}
\end{figure}


\subsubsection{Maximum Volume Ellipsoid}
\label{maximum_volume_ellipsoid_motivation}

Ideally, we would then construct the maximal volume ellipsoid within this buffered region.
However, this involves extending the formulation for a maximum volume ellipsoid from within a polyhedron to within an intersection of cones.

That is, given a set of vectors $v_i$ and scalars $0 < \beta_i < 1$ for $1\le i\le m$, define the set
\begin{align*}
P = \left\{x\bigg|\;-\frac{(x - v_i)^T}{\|x - v_i\|} \frac{v_i}{\|v_i\|} \ge \beta_i \; \forall 1 \le i \le m\right\}.
\end{align*}
We would wish to choose a $Q$ and $c$ that maximizes the volume of the ellipsoid
\begin{align*}
E = \left\{x | (x - c)^TQ(x - c) \le r \right\}
\end{align*}
subject to $E \subseteq P$.
Here, the $v_i$ represent the negative gradients.
Although we spent some time on this problem, we were not able to solve it in general.
However, we were able to construct one such ellipsoid (not maximal volume) and we were able to formulate the problem of finding the center of the maximal volume sphere.
This discussion in continued in \cref{inner_trust_regions}.
...
\end{align*}
This second definition is what is used within \cite{doi:10.1080/10556788.2015.1026968}, so that our convergence discussion must account for this difference.
...
\subsection{Inner Trust Regions}
\label{inner_trust_regions}
...
One simplification of this problem presented in \cref{maximum_volume_ellipsoid_motivation} is to construct a feasible sphere.
To compute the maximum volume sphere within the intersection of the buffering cones, we can maximize the minimum distance from the center of any cone.
...
In addition to finding the center of the maximum volume ellipsoid, we have also constructed one feasible ellipse that satisfies our requirements.
...
\iik = \{1\le i \le m | \zik \in B_{\infty}(\xk, \dk) \} \label{def_i} \\
\huk = \max_{\{w \; | \; \|w\| = 1\}} \min_{1\le i\le m} w^T \hgik\label{def_hu} \\
\inf_{k}\min\{\sigma(\nabla c_{\mathcal I_k}(\xk)) \; | \; \sigma(\nabla c_{\mathcal I_k}(\xk)) > 0 \} \ge \epsilon_{\sigma}\\
\f = \{x \in \rn | \quad c_i(x) \le 0\quad \forall 1 \le i \le m \} \label{def_feasible} \\
...
Hypothesis 0 is satisfied by the Generalized Cauchy Point \cite{Conn:2000:TM:357813} if we set $\searchtrk = \outertrk \cap \feasible$.
% as discussed in \cref{search_a_lot}.
As discussed in \cref{poised_ellipsoid}, Hypothesis H4 is satisfied by letting $\sampletrk$ have an ellipsoidal as long as the condition number of $\qk$ is bounded.
...
When the trial point is infeasible, we simply reduce the trust region radius.

However, we have also implemented a more sophisticated algorithm that modifies the trust region subproblem.
We simply add a linear constraint that ensures we do not attempt the same evaluation.
...
\begin{algorithm}[H]
    ...
    \begin{itemize}
        \item[\textbf{Step 0}] \textbf{(Initialization)} \\
            ...
            A set of feasible evaluations $H_{\text{feas}}$ of any points that lie within the inner trust region (this set may be empty) \\
			A small tolerance $\epsilon \in (0, 1)$
...
\subsubsection{Convergence of projections}

One of the steps in proving convergence is to show that the projection onto the modelled constraints convergences to the projection onto the true constraints.
This results from strengthening the work done already done in \cite{dummy:continuity}, \cite{dummy:perturbations}, \cite{dummy:hoffman}, and \cite{dummy:continuity_of_inverse}.

We give a summary of the results here.
Given matrices $G$, $D$, and vectors $g$, and $d$ and some constant $\epsilon > 0$, let
\begin{align*}
...
Then they show the following result:
\begin{theorem}
There exist positive constants $\epsilon_0$ and $c$ such that for all $\epsilon \le \epsilon_0$,  one has $\|\xo - \xo'\| \le c \sqrt{\epsilon}$.
\end{theorem}

We find an expression for these constants, to show that as the matrices $\nabla m_{c}(\xk)$ converge to $\nabla c(\xk)$ we have that the projection onto $\feasiblek$ converges to the projection onto ${\mathcal F}^{(k)}$.
