...
\begin{proof}
Give $\nabla m_{c_{\mathcal I_k} }(\xk)$ its singular value decomposition $\nabla \hat m_{c_{\mathcal I} }(\xk) = U\Sigma V^T$.
...
Let $y = \wik + ts \in \fik \cap B_{\infty}(\xk, \dk) $ with $t > 0, \|s\| = 1, -s^T\hgik \ge \beta \dk^{\frac 1 2}$.
By \cref{def_lambda_poised}, there exists a $\nu \in \rn$, $\|\nu\|\le 1$ such that 
\begin{align*}
\nabla c_i(\xk) = \nabla m_{c_i}(\xk) + \epsilon_{g}\dk^2\nu.
\end{align*}
...
Also, by \cref{def_f} we know that $-s^T \hgik \ge \beta \dk^{\frac 1 2}$ where $\|s\| = 1$, and by our assumption \cref{delta_is_small_enough}, we know
\begin{align*}
...
\end{align*}
which implies by \cref{def_g_bounds}
...
From there, we can conclude with \cref{def_lipshitz}, \cref{z_is_active}
\begin{align*}
