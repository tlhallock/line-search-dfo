...
\end{array}
\]
where $\xk$ is the current iterate, $\modelk$ is the model function approximating $f$,  and $\modelconstrainti$ are the model functions approximating the constraint functions $c_i, \forall i \in \mathcal{I} \union \mathcal{E}$, and $\Delta_k$ is the radius of the trust-region.
...
Conceptually, the model functions are ``trusted'' only within a distance $\Delta_k$ of the current iterate $\xk$; so the trust-region subproblem restricts the length of step $s$ to be no larger than $\Delta_k$.
...
Suppose that a set $S$ and a radius $\Delta_{\text{max}}$ are given.
Assume that $f$ is twice continuously differentiable with Lipshcitz continuous Hessian in an open domain containing the $\Delta_{\text{max}}$ neighborhood
$\cup_{x \in S} B(x; \Delta_{\text{max}})$ of the set $S$.
\end{assumption}

\begin{definition}
\label{fully_quadratic_definition}
Let $f$ be a function that satisfies \cref{fully_quadratic_assumption}.
A model function $m : \mathbb R^n \to \mathbb R, m \in C^2$ with Lipschitz continuous Hessian with corresponding Lipschitz constant bounded by $\nu_2$  is called a fully quadratic if there exist positive constants
$\kappa_f$, $\kappa_g$, and $\kappa_h$ such that for any $\Delta \in (0, \Delta_{\text{max}})$ and $x \in S$:
\begin{align}
...
For any $\epsilon \in (0, \frac 1 4)$, and $p+1 = \frac {(n+1)(n+2)}{2}$, \cref{model_improving_algorithm} produces a set of $p+1$ points in the unit ball $B(0; 1)$ such that the Lagrange-interpolating model is fully quadratic.
\end{theorem}

\color{red}
Algorithm 6.3
Algorithm 6.5
Combine this with previous definition
\color{black}
