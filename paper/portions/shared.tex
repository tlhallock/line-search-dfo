...
\subsubsection{Finding the Maximal $ \ellipsek $ Given $\ellipsoidcenter$}
...
\item work with the definition of fully quadratic
\end{itemize}
\color{black}
...
\begin{align}
\label{lipschitz}
\|\nabla f(x) - \nabla f(y)\| \le \|x - y\| \quad \forall x, y \in \domain
\end{align}
\end{assumption}
...
\|\nabla^2 f(x) - \nabla^2 f(y)\| \le \|x - y\| \quad \forall x, y \in \domain
\end{align}
\end{assumption}
Notice that this assumption allows us to create fully quadratic models in $\domain$.
...
\begin{itemize}
\item[H0] The efficiency condition \cref{efficiency} is satisfied.
...
H0 can be satisfied within our algorithm by selecting the Generalized Cauchy Point \cite{Conn:2000:TM:357813} to solve the trust region subproblem.
Notice that H1 and H2 are kept as hypothesis within our algorithm.
We have replaced H3 with the straightforward hypothesis  \cref{uniformly_bounded_hessians} based on our implementation.

The only assumption not yet dealth with is H4, which is the topic of \cref{satisfying_accuracy}.
...
This implies that each of the sample points used to construct $\mfk$ are output of \cref{model_improving_algorithm}, and therefore $\mfk$ is a fully quadratic model over $B_{\infty}(\xk, \dk)$.
...
\begin{lemma}
\label{replacing_h3}
Assume that \cref{other_uniformly_bounded_hessians} is satisfied, and for each $k \in \naturals$, $\mfk$ is a fully quadratic model of $f$ over $B_{\infty}(\xk, \dk)$  as in \cref{fully_quadratic_definition}.
...
% \color{red}
% \begin{itemize}
% \item[H5] For any critical point $x^{\star} \in \feasible$, we have that there exists a $d \in \Rn$ such that $d^T\nabla c_i(x) < 0, \forall i \in \activei(x^{\star})$.
% \end{itemize}
% \color{black}
% \paragraph{version 2}
% \begin{itemize}
% \item[H5] There exists an $\epsilon_X > 0$ such that for each $x \in \feasible$, we have that there exists a $d \in \Rn$ such that
% $d^T\nabla c_i(x) \le -\epsilon_X, \forall i \in \activei(x^{\star})$.
% \end{itemize}
% \paragraph{version 3}
% Our assumption reads
% \begin{itemize}
% \item[H5] There exists an $\epsilon_X > 0$ such that for each $x \in \feasible$, the vector $\hat d = \frac d {\|d\|}$ where 
% $d  = \nabla c_{\activei(x)}^T(\nabla c_{\activei(x)}\nabla c_{\activei(x)}^T)^{-1} e$ satisfies
% ${\hat d}^T\nabla c_i(x) \le -\epsilon_X, \forall i \in \activei(x^{\star})$.
% \end{itemize}
% \paragraph{version 4}
% Our assumption reads
