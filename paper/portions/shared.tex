...
\color{red}
\begin{itemize}
\item Define $\epsilon$ in the lemma.
\item Remove the unnecessary versions of the constraint qualification.
\item Fix a variable either being $u$ or $x$ in the mapping back to the unit ball.
\end{itemize}
\color{black}
...
However, our implementation allows us we can replace H3 with a more straightforward hypothesis:
...
\begin{lemma}
If $\dk \to 0$ as $k \to \infty$, then H3' implies H3.
\end{lemma}
...
We restart the theorem here, with the simplication that $f$ is deterministic function:
\begin{lemma}
\label{change_radius}
...
We then map this ellipsoid to the unit ball with an affine mapping $T$ described in \cref{shifted_ellipsoid} and create shifted functions
$\hat {m}_f(u) = m_f(T^{-1}(u))$ and
$\hat f (u) = f(T^{-1}(u))$.
After choose sample points that ensure our shifted model $\hat m _f$ is $\Lambda$ poised for some constant $\Lambda$ by using \cref{model_improving_algorithm},
we know that for all $u \in B(0, 1)$,
\begin{align*}
\left\|\nabla \hat {f}\left(u\right) - \nabla \hat{m}_f\left(u\right) \right\|\le \Lambda.
\end{align*}
Then, by using \cref{change_radius}, we know that there is a constant $\Lambda'$ such that for all $u \in B(0, 2)$:
\begin{align*}
\left\|\nabla \hat {f}\left(u\right) - \nabla \hat{m}_f\left(u\right) \right\|\le 4 \Lambda'.
\end{align*}
Finally, after defining 
\begin{align*}
\Lambda_1 = \frac{\max\{1, \alpha^{-2}\}}{\min\{1, \alpha^{-2}\}} \Lambda, \quad
\Lambda_2 = \frac{\max\{1, \alpha^{-2}\}}{\min\{1, \alpha^{-2}\}} \Lambda' \\
\end{align*}
we can use  \cref{shifted_ellipsoid} to conclude that for all $x_1 \in E$ and all $x_2 \in \{x\in\mathbb R^n | (x - c)^TQ(x-c) \le \epsilon \}$:
\begin{align*}
\left\|\nabla f\left(x_1 \right) - \nabla m_f\left(x_1\right)\right\| \le \Lambda_1 \dk, \quad
\left\|\nabla f\left(x_2 \right) - \nabla m_f\left(x_2\right)\right\| \le \Lambda_2 \dk.
\end{align*}
...
Finally, using \cref{change_radius} we know that the model is poised with a constant of $\Lambda_2$ over $\xk$ as we only double the radius.
