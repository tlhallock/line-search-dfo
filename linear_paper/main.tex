...
%$B_2(0;\delta)$.
%
%Prior to applying \cref{model_improving_algorithm},  we calculate a transformed set $\hat{Y}=\{\hat{y}^{(0)}, \hat{y}^{(1)},\ldots,\hat{y}^{(p)}\}$ by computing $\hat{y}^{(i)} = T(y^{(i)};1)$ for each $y^{(i)} \in Y$.    Observe that $\hat{Y} \subset B_2(0;\itbound)$; so  by \cref{set_is_poised}, applying 
...
Let $\hat{m}_f(u)$ be a model of the transformed objective $\hat f(u) = f(T^{-1}(u; \delta))$,
 and define $\mf(x)=\hat{m}_f(T(x;\delta))$.    Then, for any $x$, we have
 
\begin{align}
| \mf(x) - f(x)| = \left| \hat{m}_f(T(x;\delta))-\hat{f}(T(x;\delta))\right| \label{transformed_error_bounds} \\
\|\nabla  \mf(x) - \nabla  f(x)\| \le  \delta \sqrt{\|Q\|} \left\| \nabla \hat{m}_f(T(x;\delta))-\nabla \hat{f}(T(x;\delta)) \right\|  \label{transformed_error_bounds2} \\
\|\nabla^2 \mf(x) - \nabla^2 f(x)\| \le  \delta^2 \|Q\|\left\| \nabla^2 \hat{m}_f(T(x;\delta)) - \nabla^2 \hat{f}(T(x;\delta))\right\| \label{transformed_error_bounds3}.
\end{align}
 
\end{lemma}
\begin{proof}
The first result \cref{transformed_error_bounds} is trivial.   
For the gradient  bound, let $u = T(x;\delta)$.  Then, by the chain rule,
\begin{align*}
...
%For $\delta > 0$, define the transformation $T(x; \delta) = \delta D V^T\left(x - \ck\right)$.   
%
%% \sbnote{Check this.   I think we should use the transformation $T(x;1)$ instead.}
%%Notice that
%%% for some $\itbound > 1$,
...
%
%Let $m_f$ be the quadratic function that interpolates $f$ on $\Yfinal$.  Observe that the quadratic functions  $m_f$ and $\hat{m}_f \circ T(\cdot;1)$ agree on the sample points $\Yfinal$, so $m_f = \hat{m}_f \circ T(\cdot;1)$.  Thus, by \cref{Lemma 5.1},  for any $x \in \sampletrkbig$ and $y = T(x;1) \in B_2(0;\itbound)$, we have
%
%\begin{align}
