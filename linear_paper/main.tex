...
\sbnote{Changed notation for the Vandermonde matrix from $V$ to $M$ for consistency.
$V$ is used elsewhere to denote the volume of the ellipsoid and also in the eigendecomposition of  matrix.}
...
\sbnote{maybe we can delete the following}
...
\scaledellipsek &= \left\{x \in \Rn \bigg | \left(x - \ck\right)^T\qk\left(x - \ck\right) \le  2 \right\}.
\end{align*}
...
\begin{align*}
\scaledellipsek = \left\{x \in \Rn \bigg | \left(x - \ck\right)^T \qk\left(x - \ck\right) \le 1 \right\}
