...
Suppose that $f$ is Lipschitz twice continuously differentiable on $\Omega$ with Lipschitz constant $\liphess$.
Then there exists a constant $\kappa>0$, independent of $Y$ and $f$ such that if $Y$ is $\Lambda$-poised  in $B_2\left(\bar y;\Delta\right)$ for interpolation on $\polydn$,  then the following error bounds hold for all
...
$\norm{\hat{M}^{-1}} \le \kappa_{\Lambda} \Lambda$.
Let $\kappa_0$ be the constant guaranteed by \cref{error_bounds_shifted}.
Then by \cref{error_bounds_shifted}, the conclusion holds with $\kappa = \kappa_0 \kappa_{\Lambda}$.
\end{proof}
...
$\left\{\ck\right\}$ be a sequence of points in $\Rn$.  The sample trust regions defined by 
\cref{define_the_sample_region} are said to be \emph{suitable} if for all $k$,
\begin{enumerate}
\item $\sampletrk \subseteq \feasible \cap \outertrk$;
\item $\xk \in \sampletrkbig$ for a fixed constant $\itbound$; and
...
%            Ensure that the sample points are poised with respect to $ \sampletrk $ for \cref{accuracy} by calling \cref{model_improving_algorithm}.
           Construct $\mfk$ and $Y^{(k)}$ by calling \cref{model_construction_algorithm} on $\sampletrk$ and $Y^{(k-1)}$.   \sbnote{$Y^{(k-1)}$ isn't exactly right here}
...
\begin{proof}
\sbnote{What is $\hat{Y}$ in this proof?}
...
Define the transformation $T(x; \itbound) = \itbound D L^T\left(x - \ck\right)$.  \sbnote{Check this.   I think we should use the transformation $T(x;1)$ instead.}
Notice that
% for some $\itbound > 1$,
