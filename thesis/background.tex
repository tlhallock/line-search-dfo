...
Most algorithms for derivative-free optimization fall into two main categories: direct-search methods and model-based methods.
...
\section{No center evaluation}

Within the following theorem we make use of the terminology in \cite{introduction_book}.
Their analysis culminates in Theorem 3.16, which corresponds to our \cref{3_16_replacement}.


\begin{theorem}
\label{3_16_replacement}
Suppose that $f$ has a Lipschitz continuous hessian with constant $\liphess$.
For some $\Delta > 0$, let $Y = \left\{y^{(0)}, y^{(1)}, \ldots y^{(p)} \right\}$ be a set of $p+1$ points in
$B\left(0, \Delta\right)$.
...
\begin{align}
f\left(y^{(i)}\right) = m\left(y^{(i)}\right) \quad \forall i \in [0, p]. \label{nce_interpolation_condition}
\end{align}
Also, let $M(\Phi,Y)$ be constructed as in \cref{vandermonde}, and define the scaled matrix
\begin{align}
...
For a fixed $i \in [0, p]$, we can subtract \cref{define_e_f} from \cref{nce_interpolation_condition} to find that
\begin{align}
...
\begin{align*}
\sum_{0 \le j \le p} \phi_i\left(y^{(i)}\right) = f\left(y^{(i)}\right)
\end{align*}
