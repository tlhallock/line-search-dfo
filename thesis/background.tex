...
Thus, it is always possible to determine whether a point satisfies an algebraic constraint prior to attempting to evaluate any black-box functions.  As such, it is relatively straightforward to modify direct-search methods to guarantee that only feasible points are considered.     
...
\paragraph*{Relaxable black-box constraints.}  
For relaxable black-box constraints,  a variety of approaches have been considered.  One approach is to reformulate the problem as a sequence of unconstrained problems using a penalty or barrier method.    [ADD REFERENCES FROM PAGE 55/56 of LarsonMenickellyWild].      Discuss FILTER METHODS (page 55).

Another approach is to construct algebraic models of the constraint functions.     ....
In \cite{glass.cooper:sequential},  linear models of nearly active constraint functions are constructed and the iterates are accepted only if they are feasible with respect to these modelled constraints.     A similar strategy is employed in the COBYLA method of Powell \cite{powell:direct}, which builds linear models of the objective and constraint functions based on a common set of sample points.  


[Unrelaxable algebraic constraints + relaxable black-box constraints]

A variant of the MADS algorithm is proposed in \cite{burmen.olensek.ea:mesh} which uses linear regression models of the constraint functions to guide the choice of search and poll steps.  
[continue here]

In \cite{Troltzsch2016}, Tr\''{o}ltzsch proposes a model-based trust-region DFO algorithm that handles nonlinear equality constraint 
...
\paragraph*{Unrelaxable black-box constraints.}   We now turn our attention to the hardest case--that of unrelaxable black-box constraints.  In this case,  it is no longer possible to guarantee that black-box function evaluations will never be attempted at infeasible points.   However, it is desireable to minimize the occurrence of such infeasible attempts.  To accomplish this it is necessary to construct models of the constraint functions in order to guide the choice of iterates and/or sample points.    


Are there any algorithms that do this yet???
