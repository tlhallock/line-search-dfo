...
\end{align*}
For convex constraints, this condition is necessary and sufficient for optimality under regularity assumptions.
...
However, the linearization \cref{define_truefeasiblek} {\em is} still well defined.
...
However, unlike for convex constraints, points with a linearized criticality measure of $0$ are not necessarily extremum.
...
$
\left|u_i\left(y^{(i)}\right)\right| \ge \ximin\quad \forall 0 \le i \le p.
$
...
\outertrk = B_{\infty}(\xk,\dk) = \left\{x\in \Rn | \; \xki - \dk \le x_i \le \xki + \dk \quad \forall i \in [m] \right \}.
\end{align}
