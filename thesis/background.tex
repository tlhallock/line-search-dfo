...
Most of the progress in developing  DFO methods that handle unrelaxable constraints has been relatively recent, with much of the early work in this area focused on direct-search methods for {\em algebraic} constraints.  A constraint is algebraic if the constraint functions can be computed algebraically.



 and {\em black-box constraints} (which are available only through simulations, and which may not be available at all for infeasible points).      In the case of unrelaxable algebraic constraints, it is relatively straightforward to modify direct-search methods to guarantee that only feasible points are considered.   
