...
\sbnote{We also need to check on the relationship of $\Delta$ to the sample set.   I believe the error bounds given in the theorem assume that $\Delta = \max \|y^i-y^0\|$.}
...
\begin{theorem}
\label{set_is_poised}
The sample set $Y$ obtained from \cref{model_improving_algorithm} is $\Lambda$-poised for quadratic interpolation, where $\Lambda > 0$ is a constant that depends only on $\xi_{\text{min}}$ and $n$ and is inversely proportional to $\xi_{\text{min}}$.
\end{theorem}
