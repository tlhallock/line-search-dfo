...
Direct search methods easily accommodate unrelaxable constraints, as their points do not have to satisfy geometry requirements.
...
\cite{Galvan2021} develops an algorithm for constained optimization by moving the constraints into the objective.
Their work only relies on a projection operator, and therefore handles non-smooth constraints.


Applications:

The authors of \cite{Padidar2021} apply derivative free optimization to rapid-cycling synchrotron lattice design.
This includes a non-smooth objective and both linear and simulation based constraints.

[model-based]


avoiding infeasible evaluations
\cite{BMNORW2020}
The authors apply DFO to optimize the Fayans energy density functional.
...
In \cite{CONORBIT15}, the authors use an envolope to avoid infeasible trial point evaluations.
% This algorithm shares some similarities with the one presented here,


[trust region, linear models]
\cite{NOWPAC2014}
Uses path augmentation to ensure trial points are feasible.
This is done within a local convexification of the constraints that uses path augmentation to buffer the constraints.


Derivative free optimziation with unrelaxable constraints is beginning to receive attention.

\cite{Brilli2021interior} Use an interior point algorithm to solve derivative free problems with unrelaxable, black-box constraints.
...
The algorithm presented within \cite{Conejo2015} even ensures that each iterate is feasible,
