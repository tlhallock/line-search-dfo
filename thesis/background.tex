...
Within this chapter, we discuss background material relavent to our algorithm.
...
There has been a recent growth in the number of derivative free applications and algorithms.
Most of these algorithms fall into two main categories: direct-search methods and model-based methods.
Direct-search methods use comparatively simple strategies to explore the solution space often
...
It contains an explanation of how to ensure good sample set geometry for unconstrained problems, 
poisedness for unconstrained problems, and also covers other derivative-free 
methods including direct-search and line-search methods.

A review of derivative-free algorithms and software libraries can be found in \cite{miguel_review}.
...
We compare our results to the Mesh Adaptive Search algorithm implemented within the NOMAD software
\cite{Le2011a}.
...
Direct search methods easily accommodate unrelaxable constraints, as the points they evaluate have little to no geometric requirements.
However, they can attempt more infeasible evaluations as they may not accurately approximate the constraint boundary.

There are a number of algorithms that designed to avoid infeasible evalutations, although many may use infeasible function values.
These algorithms have use a number of techniques to avoid infeasible evaluations.
...
The authors of \cite{NOWPAC2014} uses a path-augmentation to ensure trial points are feasible.
...
In \cite{CONORBIT15}, the authors develop a model-based trust region algorithm that usees an envolope to avoid infeasible trial point evaluations.
...
More recently, \cite{Brilli2021interior} use an interior point algorithm to solve derivative free problems with unrelaxable, black-box constraints.

The authors of \cite{Padidar2021} apply derivative free optimization to rapid-cycling synchrotron lattice design.
The problem a non-smooth objective and both linear and simulation based constraints.
These constraints are unrelxable, because simulation is not possible with overlapping synchrotron elements.
\begin{boxedcomment}
...
Observe that linear constraint models with an $L_{\infty}$-ball instead of an $L_2$-ball ensures $\searchtrk$ is a polytope.
