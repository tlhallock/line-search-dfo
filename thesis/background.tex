...
Then, for any constant $L > 0$, there exist constants $\kappa_{h}, \kappa_{g}$, 
and $\kappa_{f}$ such that the following error bounds hold for any function $f:\Rn \rightarrow \reals$ 
...
There is a close connection between the $\Lambda$-poisedness of $Y$ and the condition number of the Vandermonde matrix associated with the monomial basis $\bar{\Phi}$ $= \{ \bar{\phi}_0, \ldots, \bar{\phi}_p\}$ $=\{1, x_1, \ldots, x_n, x_1^2/2, \ldots x_n^2/2,x_1 x_2, \ldots, x_{n-1}x_{n}\}$.
...
If a point $y^{(j)}$, $j \ge i$ can be found such that $u_i(y^{(j)})$ has a sufficiently large magnitude, 
...
\begin{algorithm}[H]
    ...
    \begin{itemize}
        \item[\textbf{Step 0}] \textbf{(Initialization)} \\
            ...
            initialize $i=1$, $\ximin > 0$, and let $\bar \phi_i(x)$ be the monomial basis for $\polydn$.
...
By, \cite[Theorem 6.5]{introduction_book}, \cref{model_improving_algorithm} terminates.
This implies the pivots in the LU-factorization are bounded below by $\ximin$:
$
...
uses the bound on $\left\|M\left(\bar \phi, Y\right)^{-1}\right\|$ to show the set $Y$ is $\Lambda$-poised.
Note that, \cite[Lemma 3.8]{introduction_book} and \cite[Lemma 3.9]{introduction_book} establish that $\Lambda$-poisedness is scale and translation invariant.
\end{proof}
...
\begin{align*}
\begin{array}{ccc}
\min\limits_{\Lambda\ge 0, y^{(i)}\in\Rn, l_i\in\polydn} & \Lambda & \\
