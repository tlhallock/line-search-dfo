...
Let $\hat{M} = M(\bar{\Phi},\hat{Y})$, and $p = \frac{(n+1)(n+2)}{2}$.
...
\begin{theorem}
\label{4_1_14}
Left $f : \Rn \to \reals$ be twice continuously differentiable in an open convex set $D \subset \Rn$,
...
% Let $w$ be a normalized right-singular vector of a non-singular $n \times n$ matrix $A$ corresponding to its largest singular value.
