...
It contains an explanation of how to ensure good sample set geometry and the concept of 
...
of Le Digabel and Wild \cite{digabel.wild:taxonomy} distinguish between whether constraints are {\em relaxable} or {\em unrelaxable}.    An unrelaxable constraint is one that must be satisfied in order to obtain meaningful information about the objective function and/or constraint functions.    Thus algorithms for unrelaxable constraints can only use function evaluations at feasible points.

For relaxable constraints, several ideas from classical nonlinear programming algorithms have been adapted to the derivative free setting.   These include penalty methods \cite{lewis.torczon:globally, lewis.torczon:direct, bueno.friedlander.ea:inexact},   and filter methods \cite{brekelmans.driessen.ea:constrained, ferreira.karas.ea:global}.     Both penalty methods and filter methods allow iterates to violate constraints, requiring feasibility only in the limit.  As such, they are viable approaches only when the constraints are relaxable. 

Most of the progress in developing of DFO methods that handle unrelaxable constraints has been relatively recent, with much of the early work in this area focused on direct-search methods.     Again following the taxonomy in \cite{digabel.wild:taxonomy},  we distinguish between {\em algebraic constraints} (for which the constraint functions can be computed algebraically) and {\em black-box constraints} (which are available only through simulations, and which may not be available at all for infeasible points).      In the case of unrelaxable algebraic constraints, it is relatively straightforward to modify direct-search methods to guarantee that only feasible points are considered.   


 
For relaxable algebraic constraints,  several ideas from classical nonlinear programming algorithms have been adapted to the derivative free setting.     

For unrelaxable algebraic constraints, the penalty and filter methods cannot be applied since they make use of infeasible iterates.  However, barrier methods are still a viable approach since the barrier forces all iterates to be feasible.   One example is an extreme-barrier approach used in \cite{lewis.torczon:pattern}

In the case of black box constraints, it is no longer possible to apply a

We distinguish between algebraic constraints (for which 


The earliest
with much of the early work in this area focused on direct-search methods.    

The mesh adaptive direct search (MADS) algorithm \cite{audet.dennis:mesh} handles unrelaxable constraints using an extreme barrier approach in which  the method effectively assigns an objective value of  $+\infty$ to infeasible points.    The algorithm for unconstrained problems is then applied to the transformed problem.    The MADS algorithm is implemented within the NOMAD software \cite{Le2011a}.

Developing model-based algorithms for constrained optimization is complica


One of the earliest algorithm for constrained DFO is COBLYA, \cite{powell:direct}.
...
This reference provides a remarkably elegant convergence proof for a class of algorithms when the constraints are convex.
Our analysis implements abstractions made in this paper and shows the implementation satisfies their requirements.

\paragraph*{Avoiding infeasible evaluations}

There is growing interest in derivative-free methods that handle unrelaxable constraints.
Direct search methods easily accommodate these constraints, as the points they evaluate have little to no geometric requirements.
However, they can attempt more infeasible evaluations as they may not accurately approximate the constraint boundary.
The algorithms described here take steps to avoid attempting infeasible evaluations, although many may use infeasible function values.

There are several strategies for avoiding infeasible evaluations.
The authors of \cite{Galvan2021} use a reformulation strategy to avoid infeasible evaluations by moving the constraints into the objective.
Their work only relies on a projection operator and therefore handles non-smooth convex constraints.
The authors of \cite{NOWPAC2014} use a path-augmentation to ensure trial points are feasible.
This is done with a local convexification of the constraints that uses path augmentation to buffer the constraints.
The authors of \cite{BMNORW2020} apply DFO to optimize the Fayans energy density functional.
The authors avoid possible infeasible evaluations by altering the objective function to include a projection onto an $L$-1 ball.
...
The algorithm presented within \cite{Conejo2015} is also model-based trust region method, and it ensures each iterate is feasible,
...
% \begin{boxedcomment}
% It is concerning that I don't understand the previous sentence.
% \end{boxedcomment}
...
To accomplish this, we require poised sample sets with a trust-region converging to zero.
...
If the feasible region $\feasible$ is convex,  a classic \cite{Conejo:2013:GCT:2620806.2621814} \cite{Conn:2000:TM:357813} criticality measure is given by
\begin{align*}
...
In the case of polynomial model functions, a rigourous analysis of model accuracy can be performed using \emph{Lagrange polynomials}.
...
Suppose that $Y$ is $\Lambda$-poised for quadratic interpolation on $B(y^{(0)}; \Delta)$.
...
that is $LC^2$ with Lipschitz constant $L$ on an open set containing $B(y^{(0)};\Delta)$:
...
A limitation of \cref{Lambda_poised_error_bounds} is that it assumes $\hat{Y}$ is contained in the unit ball $B(0;1)$.
...
& \le \Lambda \sqrt{p+1}  \max\{1, \frac 1 2 \Delta^2\} = \hat{\Lambda}.
\end{align*}
...
\begin{algorithm}[H]
    ...
    \begin{itemize}
        \item[\textbf{Step 0}] \textbf{(Initialization)} \\
            Given a  set $Y \subset B_2(0;\Delta)$, $\Delta \ge 1$ of $p+1$ points.
            Initialize $i=0$, $\ximin \in (0,1/4]$, and let $u_i(x)= \bar \phi_i(x)$ be the monomial basis for $\polydn$.
        ...
        \item[\textbf{Step 2}] \textbf{(Check threshold)} \begin{itemize}
                ...
                \item[] If $\left|u_i\left(y^{(i)}\right)\right| < \ximin$, then replace $y^{(i)} \gets \argmax_{x \in B_2\left(0; 1 \right)}\left|u_i(x)\right|$.
				\item[] If $\left|u_i\left(y^{(i)}\right)\right| < \ximin$ after this replacement,  \textbf{Stop}: $\ximin$ was chosen too small.
            \end{itemize}
...
In this case,  the algorithm stops prematurely in Step 2.
...
%Then, for any $r \ge 1$,
%\begin{align*}
%\max_{x \in B(0; r)} \left|v^T\bar\phi(x)\right| \ge \frac 1 4
%\end{align*}
%\end{lemma}
%\begin{proof}
% When $d = 1$,
...
Then,
\begin{align*}
\max_{x \in B(0;1)} \left\|v^T \bar{\phi}(x)\right\| \ge \frac{1}{4}.
\end{align*}
\end{lemma}
...
computes a set $Y$ of $p+1$ points in the ball $B(0;\Delta)$ for which the pivots of the Gaussian elimination of
...
\[ \max_{x \in B(0;1)} u_i(x) = \max_{x \in B(0;1)} \left| v^T \bar{\phi}(x)\right| = 
\norm{v}_{\infty} \max_{x \in B(0;1)}  \left|\bar{v}^T \bar{\phi}(x)\right| \ge \frac{1}{4} \norm{v}_{\infty} \ge \frac{1}{4}.\]

%\max_{x \in B(0;1)} \norm{v}_{\infty} \left|\bar{v}^T \bar{\phi}(x)\right|\max_{x \in B(0;1)}  \left|\bar{v}^T \bar{\phi}(x)\right|
%\ge 1/4.\]
...
Finally,  observe that all points in the final sample set $Y$ are selected either from the original sample set, which is contained in $B(0;\Delta)$,  or are obtained in Step 2 from within $B(0;1)$.  Since $\Delta  \ge 1$, it follows that $Y \subset B(0;\Delta)$.  
...
By \cref{terminates},  $\hat Y \subset B(0;\Delta)$ 
