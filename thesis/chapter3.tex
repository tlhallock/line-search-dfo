...
\begin{align*}
\delta^2 \ge \|T\left(x; \sqrt{2}\delta\right)\|^2 = \left\|\frac {\sqrt{2}\delta} {\sdk} DL^T(x - \ck)\right \|^2
...
Also, let $\scaledellipsek$ be defined as in \cref{define_suitable_ellipsoid}:
\begin{align*}
\scaledellipsek = \left\{x \in \Rn \bigg | \left(x - \ck\right)^T \qk\left(x - \ck\right) \le \sdk^2 \right\}.
\end{align*}

For some $\delta > 0$, define $T(x; \delta) = \frac{\delta}{\sdk} D V^T\left(x - \ck\right)$.
...
Then, with
\begin{align*}
r_{\scaledellipsek} = \max_{x \in \scaledellipsek}\|x - \ck\|, \\
\kappa_{ef}' = \kappa_{ef}, \\
...
$x \in T^{-1}\left(B_2(0, \delta)\right) = \scaledellipsek$:
\begin{align*}
...
% \end{align*}
We see that because $D_{i, i} > 0$, we have
\begin{align*}
r^2 = \frac {\sdk^2} {\lambda_{\text{min}}\left(\qk\right)} = \frac {\sdk^2} {\min_{i \in [n]} D^2_{i, i}}\\
...
= \frac{r^2}{\sdk^2} \max_{i}D_{i,i}^2 = \frac{r^2}{\sdk^2}\|D\|^2 \\
\Longrightarrow \frac{\delta} {\sdk} \|D\| = \frac{\delta}{r} \sqrt{\condition\left(\qk\right)}.
\end{align*}
