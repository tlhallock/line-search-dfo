...
that there is a bound on $\left\| M\left(\bar \Phi, \hat Y\right)\right\|^{-1}$ depending only on $p$ and $\ximin$.
Next, we consider the shifted sample set $\bar Y = \frac{\delta_r}{\delta_2} \hat Y$, where $\delta_r = \max_{x \in \scaledellipsek}\|x - \ck\|$.
