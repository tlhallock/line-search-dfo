...
%         	Iterate $\xk = y^{(0)} \in Y$
        \item[\textbf{Step 1}] \textbf{(Construct the Transformation}) \\
        	Give $\qk \succ 0$ its eigen-decomposition $\qk = VD^2V^T$, and let
...
$t \in \min_{j \in R} \left\{\frac{b_j - A_j s_i}{A_jp}  \right\}. $
...
\begin{algorithm}[H]
    ...
    \begin{itemize}
        ...
        \item[\textbf{Step 2}] \textbf{(Compute search segment)} \\
        	...
        	and distance $t \in \min_{j \in R} \left\{\frac{b_j - A_j s_{i-1}}{A_jp}  \right\}$. \\
