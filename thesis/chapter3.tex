...
%         	Iterate $\xk = y^{(0)} \in Y$
        \item[\textbf{Step 1}] \textbf{(Construct the Transformation}) \\
        	...
        	Compute $\delta = \max_{x \in \sampletrk} \|x - \ck\|$ as in \cref{define_the_sample_region}\\
			Construct the transformation $T(x) = \delta D L^T\left(x - \ck\right)$ which maps 
\begin{align*}
...
\end{align*}
        ...
        Construct the transformed sample set $\hat Y$ by computing $T\left(y^{(i)}\right)$ for each $y^{(i)} \in Y$ \\
...
\begin{boxedcomment}
Should I use a different letter than $\delta$ here so that it is not confused with $\sdk$?
\end{boxedcomment}
...
Observe that because $D_{i, i} > 0$, we can write $\delta = \frac 1 {\sqrt{\lambda_{\text{min}}\left(\qk\right)}} = \frac 1 {\min_{i} D_{i, i}}$.
This means that 
\begin{align*}
