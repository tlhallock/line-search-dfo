...
\begin{align*}
\scaledellipsek = \left\{x \in \Rn \bigg | \left(x - \ck\right)^T \qk\left(x - \ck\right) \le \sdk^2 \right\} \subseteq B_{\infty}\left(\xk, \dk\right).
\end{align*}

Define the transformation $T(x; \delta) = \frac{\delta}{\sdk} D V^T\left(x - \ck\right)$,
and let $\delta_r = \max_{x \in \scaledellipsek}\|x - \ck\|$.
Also, let $\hat m_f(u)$ be a model of the shifted objective $\hat f(u) = f(T^{-1}(u; \delta_r))$ such that,
...
\|\nabla \hat m_f(u) - \nabla \hat f(u)\| \le \kappa_{eg}\delta_r^2,  \nonumber  \\
\|\nabla^2 \hat m_f(u) - \nabla^2 \hat f(u)\| \le \kappa_{eh}\delta_r.  \nonumber
\end{align}
...
\begin{align*}
| m(x) - f(x)| \le \kappa_{ef}'\delta_r^3, \\
...
\end{align*}
With $\delta_r = \max_{x \in \scaledellipsek}\|x - \ck\|$, by \cite[Lemma 3.8]{introduction_book},
we can scale these same points by $\frac{\delta_r}{\delta_2}$ to obtain a $\Lambda$ poised set over $B_2(0, \delta_r)$.
Thus, there exist constants $\kappa_f', \kappa_g', \kappa_h'$ such that for all $u \in B_2(0, \delta_r)$:
\begin{align*}
