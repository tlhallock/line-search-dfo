...
are said to be \textbf{suitable} if all of the following are satisfied:
\begin{itemize}
...
Given some symmetric matrix $\qk \succ 0$, we can give $\qk$ an eigen-decomposition $\qk = L D^2 L^T$,
where $L^TL = I$ and $D$ is a diagonal matrix with positive entries.
...
\begin{align*}
\delta^2 \ge \|T(x; \sqrt{2}\delta)\|^2 = \left\|\frac {\sqrt{2}\delta} {\sdk} DL^T(x - \ck)\right \|^2
= \frac {2\delta^2} {\sdk^2} (x - \ck)^TQ(x - \ck) \\
\Longleftrightarrow (x - \ck)^TQ(x - \ck) \le \frac 1 2 \sdk^2.
\end{align*}
...
%         	Iterate $\xk = y^{(0)} \in Y$
        \item[\textbf{Step 1}] \textbf{(Construct the Transformation}) \\
        	Give $\qk \succ 0$ its eigen-decomposition $\qk = LD^2L^T$  \\
%         	Compute $\delta = \max_{x \in \sampletrk} \|x - \ck\| = \frac {1} {\sqrt{2}\min_{i} D_{i, i}}$ as in \cref{define_the_sample_region}\\
			Construct the transformation $T(x; \sqrt{2}) = \frac{\sqrt{2}}{\sdk} D L^T\left(x - \ck\right)$ which maps 
...
Let $\sdk>0$, $\ck \in \Rn$ and a symmetric matrix $\qk \succ 0$ be given, and
let the eigen decomposition of $\qk$ be $\qk = LD^2L^T$, $L^TL = I$, where $D$ is diagonal with positive entries.
For some $\delta > 0$, define $T(x; \delta) = \frac{\delta}{\sdk^2} D L^T\left(x - \ck\right)$.
Also, let $\hat m_f(u)$ be a model of the shifted objective $\hat f(u) = f(T^{-1}(u))$ such that,
...
\end{align*}
the model function $m_f(x) = \hat m_f(T(x))$ satisfies the following error bounds for all 
$x \in T^{-1}\left(B_2(0, \delta)\right)$:
% = \left\{x \in \Rn \bigg | \left(x - \ck\right)^T \qk\left(x - \ck\right) \le \sdk^2 \right\}$
\begin{align*}
...
% \end{align*}
Observe that because $D_{i, i} > 0$, we have
\begin{align*}
\frac{\delta}{\sdk} 
...
= \frac{\delta^2}{\sdk^2} \max_{i}D_{i,i}^2 = \frac{\delta^2}{\sdk^2}\|D\|^2 \\
\Longrightarrow \frac{\delta} {\sdk} \|D\| = \sqrt{\condition\left(\qk\right)}.
\end{align*}
...
and define $\delta = \sqrt{2}$ so
% $\delta = \max_{x \in \unshiftedellipsoid} \|x - \ck\|$, so 
the transformation $T(x; \delta) = \frac{\delta}{\sdk^2} D L^T\left(x - \ck\right)$ 
wich maps $\sampletrk = \ellipsek$ to the unit ball.
...
that there exist constants $\kappa_f, \kappa_g, \kappa_h$ such that for all $u \in B(0, \delta)$:
\begin{align*}
...
We begin by describing variants of the algorithm with different \emph{ConstructTrustRegion} subroutines.
...
Even with these modification, the iterates may approachthe boundary of the feasible region.
