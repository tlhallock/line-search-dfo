...
\sampletrk = \left\{x \in \Rn \bigg | (x - \ck)^T \qk (x - \ck) \le \frac 1 2 \sdk \right\}
\end{align}
...
\begin{algorithm}[H]
    ...
    \begin{itemize}
        \item[\textbf{Step 0}] \textbf{(Initialization)} \\
            ...
            and $\alpha > 0$.
            Choose starting  point $x^{(0)} \in \feasible$, $\Delta_0 > 0$, and set $k\gets 0$.
...
%            Ensure that the sample points are poised with respect to $ \sampletrk $ for \cref{accuracy} by calling \cref{model_improving_algorithm}.
           Select sample points within $\sampletrk$,  and construct the model function $\mfk$.       
        ...
        \item[\textbf{Step 3}] \textbf{(Solve the trust region subproblem)} \\
            Compute $\sk = \argmin_{s \in \searchtrk} \mfk\left(\xk + \sk\right)$. 
        ...
        \item[\textbf{Step 4}] \textbf{(Test for improvement)} \\
            ...
            \begin{itemize}
               ...
               set $\xkpo=\xk+\sk$,  $\dkpo \gets \omegadec\dk$
                \item If $\rk > \gammabi$,  {\em accept trial point and expand trust region}:  
                set $\xkpo=\xk+\sk$, $\dkpo \gets \omegainc\dk$   
...
$\lctrb = \begin{pmatrix}{c} \lcb \\ \xki+\dk \\ -\xki+\dk \end{pmatrix}$.
We can then define
\begin{align}
...
\sbnote{$\scaledellipsek$ is defined by radius $\delta_k$.  $\ellipsek$ is defined by radius $\delta_k/\sqrt{2}$.
...
The Hessian's of $\mfk$ are uniformly bounded.  That is, there exists a constant $\beta \ge 1$ such that
\begin{align}
...
then \cref{linearly_constrained_dfo_simple} is a particular implementation of the algorithm studied witin this reference.
...
Also, note that although \cite{Conejo:2013:GCT:2620806.2621814} presents a criticallity measure based on the true projection onto the constraints,
...
As their second assumption $H_2$, they assume \cref{lower_bound}, and their $H_3$ is \cref{uniformly_bounded_hessians_of_mf}.
...
\subsection{Simplified Third Hypothesis}
\label{simpler_h3}
...
Because of our construction, we first show the intermediate results \cref{change_radius} and \cref{shifted_ellipsoid} 
...
, the model function $\hat{m_f}$ constructed from that sample set 
...
\begin{boxedcomment}
Break out the justification of $\dmax$ into an observation before the lemma.
\end{boxedcomment}


\begin{theorem}
\label{linear_accuracy_is_satisfied}
Suppose that \cref{lipschitz_hessian}, \cref{uniformly_bounded_hessians_of_mf}, and \cref{interior_point} hold.
...
are suitable accourding to \cref{define_suitable_ellipsoid}.
...
\begin{itemize}
\item $\ellipsek = \{x \in \Rn | \left(x - c^{(k)} \right)^T \qk \left(x - c^{(k)}\right) \le \frac 1 2 {\delta_k}^2 \}$, $\ellipsek \subset \polyk$.
\item $\scaledellipsek = \{x \in \Rn | \left(x - c^{(k)}\right)^T \qk \left(x - c^{(k)}\right) \le {\delta_k}^2 \}$, $\xk \in \scaledellipsek$.
\end{itemize}
...
This along with \cref{eq:DFO-linear}, \cref{lipschitz_hessian}, \cref{uniformly_bounded_hessians_of_mf} satisfy the assumptions for \cref{change_radius}.
...
\begin{align*}
\sampletrk = \ellipsek = \left\{x \in \Rn | (x - \ck)^T \qk (x - \ck) \le \frac 1 2 \sdk^2 \right\}
\end{align*}
...
\begin{boxedcomment}
I use an arbitrary set within \cref{alphas_are_bounded}.
\end{boxedcomment}
...
We then compute
\begin{align}
\alphakp=\sqrt{\left(1+\left(\alphak\right)^2\right)\left(1+\frac{1}{\sqrt{2}}\right)} \label{define_alphakp} \\
\deltaf = \frac 1 {\alphakp} \min_{i \in [m] \setminus \activei\left(\xk\right)} {\lcb}_i - \left({\lca}_i\right)^T\xk. \label{define_deltaf}
\end{align}
...
1 & \boldsymbol0^T \\
\boldsymbol 0 & {\alphakp}^{-2} \boldsymbol I \\
\end{pmatrix} \rotk,
\quad
\ck = \xk+\deltaf \uk,
\quad
...
If $\activei = \emptyset$ during iteration $k$, then \cref{define_linear_trivial_ellipse} defines a suitable ellipsoid for iteration $k$ according to \cref{define_suitable_ellipsoid}.
\end{lemma}
% $\epsilon > 0$, $c \in \Rn$  and a positive definite, symmetric matrix $Q$, 
\begin{proof}
If $\activei = \emptyset$, then we are free to use \cref{define_linear_trivial_ellipse}.
...
Let $\activei$, $\deltaf$ $\rotk$, $T_k$, and $\polyk$ be defined by
\cref{define_activei}, \cref{define_deltaf}, \cref{define_r}, \cref{defineshiftedcone}, and \cref{polyhedron_k}
respectively.

For each iteration $k$, if $\activei\left(\xk\right) \ne \emptyset$, the ellipsoid
\begin{align}
\ellipsek = \left\{x \in \Rn \bigg | f_e\left(\deltaf, \frac 1 2 \deltaf^2, \alphak, T_k(x)\right) \le 0\right\} \label{definefeasibleellipsoid}
\end{align}
...
\cref{define_alphakp}, \cref{define_r}, \cref{defineunshiftedcone}, \cref{defineshiftedcone}, and \cref{polyhedron_k}
respectively.
...
\Longrightarrow (t - \deltaf)^2 + \frac {1} {\left(\alphak\right)^2} \|\sigma\|^2 \le \frac 1 2 \deltaf^2
\Longrightarrow (t - \deltaf)^2 \le \frac 1 2 \deltaf^2
\Longrightarrow t \le \left(1 + \frac 1 {\sqrt{2}}\right) \deltaf
\end{align*}
so that 
\begin{align*}
\|x\|^2 = t^2 + \|\sigma\|^2 \le \left(1 + \left(\alphak\right)^2 \right) t^2 \le \left(1 + \left(\alphak\right)^2 \right) \left(1 + \frac 1 {\sqrt{2}}\right) \deltaf^2 = {\alpha'}^2 \deltaf^2 \\
\Longrightarrow \|x\| \le \alphakp \deltaf \le L
\end{align*}
...
For some iteration $k$, let $\deltaf$, $\rotk$, $T_k$, $\unshiftedcone$, $\shiftedcone$, $\polyk$ be defined as in
...
The ellipsoid
\begin{align}
\scaledellipsek  = \left\{x \in \Rn | f_e\left(\deltaf, \deltaf^2, {\alphak}, T_k(x)\right) \le 0\right\} \label{definescaledfeasibleellipsoid}
\end{align}
...
We have that
\begin{align*}
f_e(\deltaf, \deltaf^2, {\alphak}, T_k\left(\xk\right) ) =  
f_e(\deltaf, \deltaf^2, {\alphak}, 0 ) = \\
...
Observe that \cref{define_linear_nontrival_ellipsoid} defines  $\ellipsek$ and $\scaledellipsek$ within \cref{define_ellipsoid_function} to be
\begin{align*}
\ellipsek = \left\{x \in \Rn \bigg | f_e\left(\deltaf, \frac 1 2 \deltaf^2, \alphak, T_k(x)\right) \le 0\right\}, \quad \textrm{and}  \\
\scaledellipsek = \left\{x \in \Rn | f_e\left(\deltaf, \deltaf^2, {\alphak}, T_k(x)\right) \le 0\right\}.
\end{align*}
...
In practice, this ellipsoid could be far less than desirable.
...
\begin{boxedcomment}
To ensure the condition number is bounded, we can give a cholesky factorization.
\end{boxedcomment}
...
\begin{boxedcomment}
Somewhere, probably not here, I should talk about the current iterate not being in $\sampletrk$.
\end{boxedcomment}
...
\subsubsection{Maximizing the Volume given a Center}
...
Let $\bar{\lctrb} = \lctrb - \lctra \ck$ and $d = x - \ck$ so that the polytope becomes
\begin{align*}
\polyk = \left\{ \ck + d \in \Rn \; | \;  \lctra d \le \bar{\lctrb} \right\}
\end{align*}
...
Because our feasible region contains an interior point, we would expect some $\ximin>0$ to allow the algorithm to run to completion.
However, for more complicated nonlinear constraints, there may not be such a $\ximin$.
...
We conjecture that this insight could give rise to yet another model improving algorithm that uses {\em full pivoting} to select only those monomials 
useful for approximating functions over $\sampletrk$.
When there is no new point/monomial to the basis that can be added to the existing set of lagrange polynomials, the algorithm would simply stop the LU factorization.
Note that the maximization over $\sampletrk$ to find a replacement point would need to maximize $|u_j|$ for several $i \le j \le p$ rather than simpy $i$.
...
This process is described in \cref{linearly_constrained_dfo_simple}
...
\begin{algorithm}[H]
    ...
    \begin{itemize}
        ...
        if $i = n_{\text{points}}$ stop, otherwise set $i \gets i+1$ and go to Step 1.
    \end{itemize}
\end{algorithm}
