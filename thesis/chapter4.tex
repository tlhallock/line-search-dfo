...
\alpha^{\textrm{hull}} = \max_{\|u\| = 1} \min_{i \in \mathcal A^{\textrm{hull}}} -u^TA_i^{\textrm{hull}} \\
\end{align*}
so that the cone
\begin{align*}
\left\{x \in \Rn | \; x = \xk + s, s^Tu^{\textrm{hull}} \ge \alpha^{\textrm{hull}}\left\|s\right\| \right\}
\end{align*}
is contained within $P_{\textrm{hull}}$ near $\xk$.

We are then free to construct the ellipsoid
\begin{align*}
f_e(\epsilon, \delta, \theta; x) = (x - \epsilon e_1)^T\begin{pmatrix}
1 & \boldsymbol0^T \\
\boldsymbol 0 & \frac{\theta^2}{1 - \theta^2} \boldsymbol I \\
\end{pmatrix}(x - \epsilon e_1) - \frac 1 2 \delta^2
\end{align*}
and rotation matrix
\begin{align*}
\rotk = 2\frac{(e_1 + \huk)(e_1 + \huk)^T}{(e_1 + \huk)^T(e_1 + \huk)} - \boldsymbol I
\end{align*}
by defining
\begin{align}
\gamma &= 1 + \frac 1 {\sqrt{2}} \\
...
The ellipsoid output from \cref{restore_feasible_ellipsoid} is conditioned, trusted, adjacent, and non-empty according to \cref{ellipsoids_notation_definitions}.
\end{lemma}
\begin{proof}
\end{proof}
