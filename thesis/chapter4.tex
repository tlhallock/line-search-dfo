...
$\capcones \cap B_\infty(\xkpo,\Delta_{k+1}) \subset \feasiblek$.     \sbnote{verify that this last statement is correct.  Do we actually prove that somewhere?}
...
\end{itemize}
\end{definition}
\sbnote{In the definition above, the statement ``if an ellipsoid defined by $\qk,\ck,\delta_k$ ...'' is confusing, since it is defined two different ways.   In particular, in the definition of ``trusted'', this ellipsoid is $\unshiftedellipsoid$ whereas in the definition of ``adjacent'', it is $\scaledunshiftedellipsoid$.}
...
The above results will be used in \cref{convergence} to prove that for small enough $\Delta_k$, the sample trust region $\sampletrk$ will be guaranteed to be feasible.    But for large $\Delta_k$, there is no such guarantee, so we must consider how to recover when an infeasible sample point is chosen.   This section describes methods for recovering a feasible ellipsoid.  
