...
Additionally,  from the definition of $\activeconstraintsk$, any point that is infeasible with respect to a model constraint not in $\activeconstraintsk$ must be too far away from $\xk$ to lie within  $B_\infty(\xk,\dk)$.
Thus,  $\capcones \cap B_\infty(\xk,\dk) \subset \feasiblek$.
Moreover,   the multiplier $(1+K_a)$ used in the definition of $\activeconstraintsk$ will also allow us to prove that
$\capcones \cap \outertrkpo \subset \feasiblek$.     \sbnote{verify that this last statement is correct.
Do we actually prove that somewhere?}
...
\sbnote{Maybe we need to be consistent about writing $\outertrk$ versus $B_{\infty}(\xk,\dk)$.}
...
We then present two methods for constructing $\sampletrkpo$.
Finally, we show that the conservative ellipsoid method discussed in \cref{the_conservative_ellipsoid} satisfies certain properties that will be
needed in our convergence proof.
...
Ideally, we would like the sample trust region to be the largest possible ellipsoid contained within $\capcones \cap \trk$.   To accomplish this, we first need to understand how to check whether an ellipsoid is contained in a cone.}



Ideally, we would like the sample trust region to be the largest possible ellipsoid contained within $\capcones \cap \trk$.   
%Although this sounds appealing, we were only able to formulate this problem as a multilevel optimization problem.    
This problem can be formulated as a multilevel optimization problem.  In particular, we first describe how to check whether a given ellipsoid is contained
within any particular cone.    This requires solving a lower-level optimization problem.
...
The above results will be used in \cref{ellipsoid_is_feasible_section} to prove that for small enough $\dk$, the sample trust region $\sampletrk$ will be guaranteed to be feasible.
...
\subsection{Convex feasible region}  If the feasible region is convex,  (for example, when the constraints are quasi-convex),
...
\subsection{Nonconvex feasible region}    \sbnote{I suggest removing this section and focusing only on the case when the feasible region is convex.}
...
In the case when the feasible region is not convex, one must ask the question as to whether an acceptable ellipsoid even exists.   This section section answers that question in the affirmative if we assume that the following regularity condition is satisfied.
...
If the trial point is infeasible,  we simply decrease the trust region radius


 is to simply decrease the trust region radius.
