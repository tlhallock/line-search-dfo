...
Under regularity assumptions, this criticality measure is $0$ for points satisfying the first order necssary conditions for optimality.
...
\item We show that the accuracy condition \cref{accuracy} is satisfied for a adjacent, conditioned, non-empty ellipsoids as defined in \cref{ellipsoids_notation_definitions}. \\
...
Note that this inequality only needs to be satisfied for constraints that are nearly active.
...
Observe that if this limit does not hold, then there exists an $\epsilon_{\textrm{lb}} > 0$ such that the set
\begin{align}
...
In the next section, we make use the previous result to show agreement between our criticality measure
...
Also, our implementation avoids calling the restore feasible sample region when possible by scaling the previous sample region to lie within the current trust region.
...
In order to alleviate descent directions being removed from $\capcones$, 
...
Notice that the algorithm evaluated enough points to construct quadratic models the constraints, because it constructed a quadratic model for the objective.
...
\begin{array}{ccc}
\min_{s \in \outertrk} & \mfk\left(\xk + s\right) & \\
\textrm{s.t.} & q_{c_i}^{(k)}\left(\xk + s\right) + \beta \dk^{p_{\beta}}\left\|s - \xk\right\|^2 \le 0 & \forall i \in \activeconstraintsk.
\end{array}
\end{align*}
...
Our implementation decreases the trust region when in infeasible trial point is found.
Another strategy that may allow for larger trust region radaii is to solve the trust region subproblem with again, 
...
For each problem in the problem set, we assigned a starting ellipsoid.
...
Of course, we have function values for all functions within the Hott-Schittkowski library:
...
NOMAD handles infeasible evaluations explicitely by including a flag for when the evaluation failed.
...
There weere multiple approaches for accomplishing this when an infeasible point $x$ was attempted.
