...
%We use the same algorithmic framework as was presented in \cref{chap:linear}.    As we did there, we define three trust regions:  the outer trust region $\outertrk = B_2\left(\xk;\dk\right)$, the sample trust region $\sampletrk$ from which the sample points are selected, and the search trust region $\searchtrk$, which defines the feasible region for the trust region subproblem.
%
...
\end{align}
\item The tuple $\left(\qkpo, \ckpo, \sdkpo\right)$ is \emph{trusted} for $\xkpo, \dkpo$ if an ellipsoid defined by 
$\qkpo$, $\ckpo$, $\sdkpo$ lies within $ \capcones \cap \trkpo $. 
That is,
\begin{align}
...
% \sbnote{Do you have a reference to justify this statement? }
...
Thus, we are left with projecting of an arbitrary point $(t, y) \in \Rn$ and the second order cone:
...
% Because we analyze this in more depth, we dedicate \cref{constructing_and_analysizing_conservative_ellipsoid} to this.
...
\begin{algorithm}[H]
    ...
    \begin{itemize}
        \item[\textbf{Step 0}] \textbf{(Initialization)} \\
            Let $\hullpoly = \emptyset$ and $Y = \left\{y^{(0)}, y^{(1)}, \ldots, y^{(p)}\right\}$ be the previous sample points.
...
% \end{array}
% \end{align*}
% After using \cref{model_improving_algorithm} to choose sample points, we know by  that
% \end{aligorithm}
...
\end{array}
\end{align*}
Just as in \cref{linear_accuracy_is_satisfied}, after using \cref{model_improving_algorithm} to choose sample points, we know by \cref{set_is_poised} and \cref{3_16_replacement} that there
...
\end{align*}
Because the last term is positive, and $\nu^T \frac{\gmcik}{\|\gmcik\|} \le 1$, we can scale the last term to yield:
\begin{align*}
...
Here we discuss the numerical results of this algorithm when run on the Hock-Schittkowski problem set \cite{Schittkowski1981MoreTE}.
