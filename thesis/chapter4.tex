\sbnote{Questions/discussion points:
\begin{enumerate}
\item State the bilevel program for the maximum volume ellipsoid.
...
\end{align}
The starting ellipsoid,  $T_{\text{interp}}^{(1)}$,  is specified by the user.
...
\begin{algorithm}[H]
	...
	\item[\textbf{Step 1}] \textbf{(Construct ellipsoid)} \\
	   Construct an ellipsoid within $P_{\textrm{hull}} \cap \trk$ \\
    \end{itemize}
\end{algorithm}
...
This means the interier of their convex hull is non-empty.
...
\subsection{Nonconvex feasible region}
...
The only other evaluation is the trial point.
Our approach for infeasible trial points is to decrease the trust region radius, which is an acceptable approach, 
as long as we can ensure that no trial point will be infeasible for sufficiently small $\dk$: that is, $\sampletrk \subseteq \feasible$.
...
Thus, we set $ \searchtrk = \capcones \cap \trk$ by defining the trust region subproblem as
\begin{align}
...
Notice that be not searching all points within $\outertrk \cap \feasible$, we risk not providing sufficient reduction.
However, in \cref{sufficient_reduction_theorem} we show the existence a point within $ \searchtrk $
that satisfies the efficiency condition \cref{efficiency}.
...
$\tolcrit, \tolrad$ in \cref{criticality_measure_section}; % by \cref{define_algorithm_tolerances};
$\gammasm$, $\gammabi$, $\omegadec$, and $\omegainc$ were introduced in \cref{rhosection};
...
0 < \kappa_{\chi}; \; 0 < p_{\Delta} < \min\{p_{\alpha}, p_{\beta}\} \le 1; \nonumber \\
k \gets 0; \; \xinit \in \feasible; \; \Delta_0 > 0; \; T^{(0)}_{\textrm{sample}} \subseteq B_{\infty}\left(\xinit, \Delta_{0}\right) \cap \feasible \nonumber 
        \end{align}
...
%         by \cref{define_ellipsek} if $\activeconstraintsk \ne \emptyset$ and \cref{define_trivial_ellipsek} if $\activeconstraintsk =\emptyset$.
        ...
        \item[\textbf{Step 2}] \textbf{(Check stopping criteria)} \\
            ...
            Compute $\chik$ as in \cref{define_criticality_measure}. \begin{itemize}
                ...
                \item[] Otherwise, if \cref{criticality_check} is not satisfied, then \\
...
We shall show that the model criticality measure converges to the true criticality measure
\begin{align}
\label{verbiage_hoffman_use_one}
\chik \approx \chi (\xk).
\end{align}
For some results, we also show that the criticality \cref{define_criticality_measure} measure converges across iterations:  \sbnote{bad sentence--why is criticality measure broken up by a cross reference?}.
\begin{align}
...
% \end{itemize}
\item Show the limit of the true criticality measure is zero \\
This is done within \cref{limit_of_true_criticality}.
...
\dk \le \dsr \label{sr_delta_small_enough}
\end{align}
then there exists $v \in \capcones \cap \trk$ such that
\begin{align*}
m_f^{(k)}(\xk) - m_f^{(k)}(v)
...
Notice that because $u_{\textrm{GC}} \in B_2\left(\xk, \dk\right)$, as long as $t \le \frac 1 2 \dk$, we will have $v(t) \in \trk$.
...
We have shown that $v(t) = u_{\textrm{GC}} + t \minangledirk \in \capcones \cap \trk$ satisfies sufficient reduction 
...
% % \begin{align*}
% % \mfk(\xk) - \mfk(u_{\textrm{GC}}) \ge \kappa_f \chik \min\left\{ \frac{\chik}{1+\|\nabla^2 \mfk(\xk)\|}, \frac 1 2 \dk, 1 \right\}.
% % \end{align*}
% 
% 
% % \begin{boxedcomment}
...
\label{limit_of_criticality_to_zero}
...
Here we discuss the numerical results of this algorithm.

\subsection{Implementation Details}
\subsubsection{Parameters}
In practice a maximum value of $\alpha$ and $\beta$ can be used for large $\dk$.
Namely, it is possible to define a $\beta_{\textrm{max}}, \alpha_{\textrm{max}} \in (0, 1)$ and compute
\begin{align*}
...
Ideally, this would use information returned from the inner optimization program to more intelligently expore ellipsoids.
...
In order to aleviate descent directions being removed from $\capcones$, we experimented with replacing the second order cones with quadratic buffers.
...
up to discrepencies between the code and publication.
...
\begin{boxedcomment}
Define what iteresting means and give performance plot examples.
\end{boxedcomment}
...
This is because the buffering cones limit descent directions, and effictively place a cap on the largest possible trust region radius.
