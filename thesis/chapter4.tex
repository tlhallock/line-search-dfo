...
\left\{x \in\Rn | \left(x - \xinit\right)^T Q^{(0)} \left(x - \xinit\right) \le \delta_0 \right\} \subseteq B_{\infty}\left(\xinit, \Delta_{0}\right) \cap \feasible 
\end{align}
...
\end{align}
to ensure the trust region radius does not remain large while approaching criticality.
Here,
$p_{\alpha}$ and $p_{\beta}$ are introduced by \cref{define_abpab},
while $\chik$ is introduced in \cref{define_criticality_measure}.
...
and $\alpha$, $\beta$, $p_{\alpha}$, and $p_{\beta}$ by \cref{define_abpab}.
...
%             Compute $\chik$ as in \cref{define_criticality_measure}.
			...
			Otherwise, if \cref{criticallity_check} is not satisfied, then \\
...
For convex constraints, we can let $\chi\left(x\right) = \left\|x - \proj_{\feasible}\left(x- \nabla f\left(x\right)\right)\right\|$ be the criticality measure for \cref{the_dfo_problem}, 
>>>>>>> cb1dc58190787c85d5f4522a66dfe4170f7b88ee
...
\end{align*}
in accordance with \cref{define_criticality_measure}: $\chik = \chi_m\left(\xk\right)$.  \sbnote{The model criticality measure above depends on the iterate $k$, so should probably have a superscript.  For example,
...
include references to their definitions when used.  \sbnote{something is wrong with this sentence.}
...
However, we show such a point exists for those iterates with $\chik \ge \kappa_{\Delta}\dk^{p_{\Delta}}$ and sufficiently small $\dk$.
