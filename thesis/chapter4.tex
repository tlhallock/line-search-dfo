...
\sbnote{Questions:
\begin{enumerate}
\item If you are using $\searchtrk = \capcones$, does this alter whether your algorithm is a special case of Conejo's algorithm?
\item Conejo's paper assumes that $\mfk(\xk) = f(\xk)$.  This means that $\xk$ {\em must} be a sample point.   Do you always use $\xk$ as a sample point?
\item Is it a problem that $\mcikmo \ne c_i(\xk)$?
\item  If $c_i(\xk) \ne 0$ and $\|gmckikmo\| = 0$, then $i \not \in \activeconstraintsk$, correct?  If that is the case, how can you be sure that the $i$th constraint will be satisfied for all $x$ in $\capcones \cap \outertrk$?
\item See question at the beginning of \cref{handling_linear_constraints_within_ellipsoid_programs}
%\item Are you using $B_\infty$ or $B_2$ for the outer trust region?
%\item $\sampletrk \subset \mathcal{F}_{m}^{(k-1)}$ not $\feasiblek$,  whereas $\searchtrk \subset \feasiblek$, correct?
%\item Please clarify what you are using for the search trust region in this chapter.  In some places, it seems like $\searchtrk = \feasiblek \cap \outertrk$, but it other places it seems like $\searchtrk = \capcones.$
\end{enumerate}
}

\sbnote{Notes for Trever:
\begin{enumerate}
\item I made changes to clarify that at iteration $k$,  the sample set $Y^k$ is chosen from $\sampletrk$ which is constructed to lie within 
$\capcones \cap \outertrk$, where $\capcones$ is defined using the model constraint $\mcikmo$ defined during the previous iteration.
\end{enumerate}
}
...
To define a trust-region method for solving this problem,  we approximate $f$ at iteration $k$ by a quadratic interpolation model $\mfk$ and each constraint function $c_i,  i=1,\ldots,m$ by a linear interpolation model $\mcik$.     Specifically,  these functions have the form
...
As discussed above, both the sample trust region and the search trust will be defined based on a set of buffering cones associated with the nearly active constraints.    The nearly active constraints are identified by using the constraint models from the previous iteration.    To make this precise, 
...
\sbnote{Note that I changed $\gmcik$ to $\gmcikmo$ in the above definition.}

\sbnote{Need to make it clear what $\infty$ means in the above definition}
\begin{boxedcomment}
...
\end{align}
\sbnote{You need to define $[m]$ before you use it.}
...
\sbnote{Add a comment that as $\Delta_k$ gets smaller and smaller, the cones get wider and wider.}
...
\sbnote{Question:  If $c_i(\xk) \ne 0$ and $\|\gmcikmo\| = 0$, then $i \not \in \activeconstraintsk$, correct?  If that is the case, how can you be sure that the $i$th constraint will be satisfied for all $x$ in $\capcones \cap \outertrk$?}
...
\sbnote{I changed $\sampletrkpo$ to $\sampletrk$ and $\outertrkpo$ to $\outertrk$ above.}
...
We will show that one of these strategies (in particular, the strategy described in \cref{conservative_ellipsoid}) has the property that 
...
\end{align}
\sbnote{double check that $\frac{1}{2} \sdk^2$ is correct.  The sentence preceding the definition says that it has radius $\delta_k$, which is not the case in the definition.}
...
\sbnote{Maybe move the definitions of the desired properties (i.e., \cref{ellipsoids_notation_definitions}) to here.}
...
\sbnote{Let's talk about this.   Here was my understanding:    the model functions $\mcik$ are constructed from a sample set chosen from $\sampletrk$.  This sample trust region is constructed so that $\sampletrk \subset \feasiblekmo \cap B_{\infty}(x^{(k)},\Delta_k)$.  Notice that the center of the outer trust region is $x^{(k)}$, not $x^{(k-1)}$.}
