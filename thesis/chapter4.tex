...
Our algorithm decreases the trust region radius when it encouters such an infeasible trial pointm,
...
\begin{array}{ccc}
\slin = \argmin_{s \in \Rn} & m_{f}(\xk + s) & \\
\textrm{s.t.} & m_{c_i}(\xk + s) \le 0 & i \in [m]\\
& s \in \trk. & \\
\end{array}
\end{align}
...
\begin{array}{ccccc}
\slin &\approx& \argmin_{s} & m_f(\xk + s) & \\
...
m_f^{(k)}(\xk) - m_f^{(k)}(\slin ) \ge \kappa_f' \chik \min\left\{ \frac{\chik}{1+\left\|\nabla^2 \mfk(\xk)\right\|}, \frac 1 2 \dk, 1 \right\}
\end{align}
...
Because we decrease the trust reigon radius when this check is not satisfied,
...
\chik \approx \chi (\xk).
\end{align}
For some results, we must also show that the criticallity \cref{define_criticality_measure} measure converges across iterations:
\begin{align}
...
\begin{align*}
\begin{array}{cccc}
& m_f^{(k)}(\xk) - m_f^{(k)}(v) & \ge & \kappa_f \chik \min\left\{ \frac{\chik}{1+\left\|\nabla^2 \mfk(\xk)\right\|}, \dk, 1 \right\} \\
\textrm{and} \quad & m_f^{(k)}(\xk) - m_f^{(k)}(v) &\ge& \tolsr\left[m_f^{(k)}(\xk) - m_f^{(k)}(\slin)\right].
\end{array}
\end{align*}
...
Similarily, because $\slin $ is feasible with respect to the linearization of the constraints, we have
\begin{align}
