...
\begin{figure}[ht]
    ...
    \caption{
    	\emph{Short:} An example of a buffering cone.
    	\emph{Long:}
...
\Longleftrightarrow \left\|\xk - \frac{c_i\left(\xk\right)}{\left\|\nabla c_i\left(x\right)\right\|^2} \nabla c_i\left(x\right)  - \xk\right\|_{\infty} \le \minangledelta \\
\Longleftrightarrow \left|c_i(x)\right| \le \minangledelta \left\|\nabla c_i(x)\right\|_{\infty} \\
\Longleftrightarrow \left|c_i(x)\right| \le \minangledelta \left\|\nabla c_i(x)\right\| \\
\end{align*}

\begin{assumption}
\label{true_constraints_minangle_assumption}
There exists $\minangledelta > 0$ and $\minanglealpha > 0$ such that for any $x \in \Omega$,
there is a unit vector $\minangledirk \in \Rn$ such that 
for any $i$ with
\begin{align*}
...
we also have
\begin{align*}
-\nabla c_i(x)^T\minangledirk \ge \minanglealpha \left\| \nabla c_i(x) \right\|.
\end{align*}
...
Suppose that \cref{true_constraints_minangle_assumption} holds.

There exists $\epsilon_{\textrm{max}} > 0$ such that for each $\epsilon < \epsilon_{\textrm{max}}$, the ellipsoid
\begin{align*}
\left\{\right\} \subseteq \feasible
\end{align*}
is, trusted, adjacent, and non-empty according to \cref{ellipsoids_notation_definitions}
\end{theorem}

\begin{proof}
Let $y \in \Rn$ with $c_i(y) = 0$.


First, suppose that $i$ is such that $\left|c_i(x)\right| > \minangledelta \left\|\nabla c_i(x)\right\|.$
By \cref{bounded_gradients_lemma}, there exists $\maxgrad > 0$ such that
\begin{align}
\label{fufeeq1}
0 = c_i(y) \le c_i(x) + \|y - x\| \maxgrad \Longrightarrow \|y - x\| \ge \frac{-c_i(x)}{\maxgrad}.
\end{align}
By \cref{mingradassumption}, there are constants $\mingradepsilon$ and $\mingrad$ such that if $-c_i(x) = \left|c_i(x)\right| \le \mingradepsilon$, then 
$\| \nabla c_i(x) \| \ge \mingrad$.
If $-c_i(x) > \mingradepsilon$, \cref{fufeeq1} becomes $\|y - x\| > \frac{\mingradepsilon}{\maxgrad}$, otherwise it is
\begin{align*}
\|y - x\| \ge \frac{\minangledelta \left\|\nabla c_i(x)\right\|}{\maxgrad} \ge \frac{\minangledelta \mingrad}{\maxgrad}.
\end{align*}
Because one of these cases must be true, we see
\begin{align*}
\|y - x\| \ge \frac 1 {\maxgrad}\min\left\{\mingradepsilon, \minangledelta \mingrad\right\}.
\end{align*}

Now, suppose that $i$ is such that 
$\left|c_i(x)\right| \le \minangledelta \left\|\nabla c_i(x)\right\|$.
Then by \cref{true_constraints_minangle_assumption}, there exists $\minangledirk$ with
\begin{align*}
-\nabla c_i(x)^T\minangledirk \ge \minanglealpha \left\| \nabla c_i(x) \right\|.
\end{align*}
...
By \cref{bounded_hessians_assumption}, we see that there exists $\maxhessian > 0$ with
\begin{align*}
c(y) \le c(x) + \nabla c(x)^T (y - x) + \maxhessian \|y - x\|^2.
\end{align*}

Suppose that $-c_i(x)\le \mingradepsilon$, so that \cref{mingradassumption} provides $\left\|\nabla c_i(x)\right\| \ge \mingrad$.
Now, let $0 < t \le \frac {\minanglealpha \left\|\nabla c_i(x)\right\|}{\maxhessian\left(2 + \minanglealpha\right)}$ be arbitrary,
and suppose that $s \in \Rn$ satisfies $s^T\minangledirk = 0$, $\|s\|\le \frac 1 2 \minanglealpha t$.
We can multiply
\begin{align*}
t \le \frac {\minanglealpha \left\|\nabla c_i(x)\right\|}{\maxhessian\left(2 + \minanglealpha\right)}
\Longrightarrow 0 \ge -\minanglealpha \left\|\nabla c_i(x)\right\| +  \maxhessian\left(2 + \minanglealpha\right) t
\end{align*}
by $\frac t 2$ to find
\begin{align*}
0 \ge -\frac 1 2 \minanglealpha \left\|\nabla c_i(x)\right\| t +  \maxhessian\left(1 + \frac 1 2 \minanglealpha\right) t^2 \\
\ge 0 -  t\minanglealpha \left\| \nabla c_i(x) \right\| + \left\| \nabla c(x)\right\| \left\|s\right\| + \maxhessian\left(t^2 + \|s\|^2\right) \\
\ge c(x) + \nabla c(x)^T (t \minangledirk + s) + \maxhessian \|t \minangledirk + s\|^2 \\
\ge c(x + t \minangledirk + s).
\end{align*}
