...
$\sampletrk$, which is defined prior to the start of the $k$th iteration.  
...
In the next section we define the concept of a nearly active constraint and define buffering cones associated with each nearly active constraints.  
...
\end{align}
The starting ellipsoid,  $T_{\text{interp}}^{(0)}$,  is specified by the user.
...
for example, adjacency could be dropped by adding a requirement that the longest axis of the ellipsoid is a percentage of the outer trust region radius.
...
= \left(v^{(i)}\right)^T \qk  v^{(i)} + t d^T \qk  v^{(i)} - \left(\ck\right) ^T \qk  v^{(i)} + t \left(v^{(i)}\right)^T \qk  d \\ + t^2 d^T \qk  d - t \left(\ck\right)^T \qk  d - \left(v^{(i)}\right)^T \qk  \ck  - t d^T \qk  \ck + \left(\ck\right) ^T \qk \ck - 1\\
= \left(d^T\qk d
\right) t^2 + \left(
d^T \qk  v^{(i)} +  \left(v^{(i)}\right)^T \qk  d - \left(\ck\right) ^T \qk  d - d^T \qk  \ck 
\right) t \\ +  \left(
...
- \qk  \left(\left(v^{(i)}\right)^T \qk  v^{(i)} + \left(\ck\right) ^T \qk  \ck  - 2 \left(\ck\right) ^T \qk  v^{(i)} - 1\right),
\end{align*}
we find that that these directions satisfy
\begin{align*}
4\left(\left(v^{(i)}\right)^T \qk  d - \left(\ck \right) ^T\qk d\right)^2 \\
- 4 \left(d^T\qk d\right) \left(\left(v^{(i)}\right)^T \qk  v^{(i)} + v ^T \qk  \ck  - 2 \left(\ck\right) ^T \qk  v^{(i)} - 1\right) \\
= d^TM_id = 0.
\end{align*}
...
Notice that by \cref{Lambda_poised_error_bounds} if a set of points is $\Lambda$-poised, their Vandermonde matrix cannot have $0$ determinant.
...
It is possible that as the trust region becomes small, we only have a single feasible evaluation in proximity.
...
With the adequate choices of directions, we believe this algorithm would eventually find a feasible ellipsoid.
...
In the case when the feasible region is not convex, one must ask the question as to whether an acceptable ellipsoid even exists.
This section section answers that question in the affirmative if we assume that the following regularity condition is satisfied.
...
\cref{sufficient_reduction_theorem} shows there is a point satisfying satisfying \cref{efficiency} 
...
Large values of $\tolsr$ ensure sufficient reduction, but may force the trust region radius to decrease prematurely.
...
Thus, we also compute a solution to \cref{original_tr_subproblem}, which does satisfy sufficient reduction.
An example of such a point is the generalized Cauchy point $s_{\textrm{GC}}$ as described in \cite{Conn:2000:TM:357813}.
Thus, with $\hat s$ an approximate solution to \cref{original_tr_subproblem},
...
Note that we reuse several other parameters previously introduced:
\begin{align}
...
%         to ensure the current sample set $Y^{(k)}$ satisfies \cref{accuracy}.
        Construct $\mfk$ and $\mcik$ with \cref{reg} after calling \cref{model_improving_algorithm} on $\sampletrk$. \\
        ...
        \item[\textbf{Step 2}] \textbf{(Check stopping criteria)} \\
        	Compute
        	$\activeconstraintsk$, $\zik$, $\wik$,  $\chik$ 
        	by \cref{define_activeconstraints}, \cref{define_z}, \cref{define_w}, \cref{define_criticality_measure}. \\
...
%             Compute $\chik$ as in \cref{define_criticality_measure}.
			...
			Otherwise, if \cref{criticality_check} is not satisfied, then \\
        ...
        \item[\textbf{Step 3}] \textbf{(Solve the trust region subproblem)} \\
        	Compute $\huk$, $\thetamink$, $\bsk$, $\fik$, $\capcones$, $\rotk$, $\slin$, and $\sk$ by
        	\cref{define_u}, \cref{define_thetamink}, \cref{define_bsk}, \cref{define_fik}, \cref{define_capcones}, \cref{define_rotation}, \cref{half_tr_subproblem} \cref{capcones_tr_subproblem}. \\
...
\begin{boxedcomment}
You wanted all of the definitions in Step 3, just to know when they were calculated, do I still need them?
\end{boxedcomment}



\sbnote{No, this is too tedious.  How about the following for Step 3:  Construct $\capcones$ by \cref{define_capcones} and solve the trust region subproblem \cref{capcones_tr_subproblem} for $\sk$.  If $\sk \not \in \feasible$, etc.}


\sbnote{You could also simplify Step 2 by saying ``Compute $\chik$ by  \cref{define_criticality_measure}.  If $ \chik < \tau_{\xi} $ etc.}
\color{red}
...
then we can let the criticality measure be
$\chi\left(x\right) = \left\|x - \proj_{F_c(x)}\left(x- \nabla f\left(x\right)\right)\right\|$.
...
\begin{align*}
\chi_m\left(x\right) = \left\|x - \proj_{\feasiblek}\left(x- \nabla \mfk\left(x\right)\right)\right\|.
\end{align*}
In accordance with \cref{define_criticality_measure}, we define $\chik = \chi_m\left(\xk\right)$.  \sbnote{The model criticality measure $\chi_m(x)$ above depends on the iterate $k$, so should probably have a superscript.  For example,
$\chi_m^{(k)}\left(x\right)$.}
...
\item Show that $\lim_{k\to\infty} \chik \to 0$ whenever $\gammabi > 0$. \\
This is done within \cref{limit_of_criticality_to_zero}.
...
\cref{ellipsoids_notation_definitions} and \cref{conservative_ellipsoid_construction} and
include references to their definitions when used.  \sbnote{something is wrong with this sentence.  Did you mean the following?
\\
Other quantities such as $\activeconstraintsk$, $\huk$, $\thetamink$, $\qk$, $\ck$, and $\sdk$ 
...
\sbnote{You should clarify that you only need this assumption in the case of a nonconvex feasible region.   For convex feasible region, you describe the \emph{RestoreFeasibiity} algorithm}.
\begin{assumption}
...
This is required to show the accuracy condition \cref{accuracy} is satisfied, because this condition number determines the sample set's poisedness, as discussed in \cref{geometry}.
...
$\qkpo$ be defined either by \cref{define_trivial_ellipsek} if $\activeconstraintsk = \emptyset$ or by \cref{conservative_ellipsoid_details} if $\activeconstraintsk  \ne \emptyset$.  Let $\minanglealpha$ be defined by \cref{minangleassumption_alt}.
...
Suppose that $\qkmo$, $\ckmo$, and $\sdkmo$ are non-empty according to \cref{ellipsoids_notation_definitions}.  \sbnote{You haven't defined what it means for each of these individual objects to be non-empty.  Instead, you defined what it means for the ordered triple $(\qkmo,\ckmo,\sdkmo)$ to be non-empty.}
\input{assumptions.d/accuracy_is_satisfied_lemma}
\replace{Fix an iterate $k$, and }{},  \sbnote{you've already assumed a fixed $k$ in the statement of the Lemma} Suppose that the sample points chosen for iteration $k$ are feasible.
...
\sbnote{At this point, you have finished proving that the accuracy condition is satisfied for sufficiently small $\Delta_k$.    So you should end the subsection here.  The next two lemmas seem to fit better with the next subsection (Sample Region Feasibility).   I suggest the following transition:}

\color{blue}
The above theorem shows that the accuracy condition \cref{accuracy_condition} is satisfied for sufficiently small $\Delta_k$.   
We now turn our attention to the feasibility of the sample region.

---Start Sample Region Feasibility Section Here---

This section show that for sufficiently small $\Delta_k$, the conservative ellipsoids defined by 
\cref{define_ellipsek} are feasible as defined by \cref{ellipsoids_notation_definitions}:
...
\sbnote{The last two lemmas of this section are the main result of the section.  I suggest combining these two into a single ``theorem'', and replacing $\unshiftedellipsoid$ by $\sampletrk$.   So you can say cleanly that for small enough $\Delta_k$, both $\sampletrk \subset \feasible$ and $\searchtrk \subset \feasible$.}
...
\sbnote{Remove this note?   Alternatively, reword it to something like:  ``The following analysis follows that in 
 \cite{Conejo:2013:GCT:2620806.2621814}, where $\eta$ corresponds to $\gammasm$, etc.''}
\paragraph*{Note:} Within \cite{Conejo:2013:GCT:2620806.2621814}, 
...
Also, assume that $k_1$ is sufficiently large that $\dk \le \dfeas$ as defined in \cref{searchtrk_is_feasible}
so that neither a sample point nor the trial point can be infeasible.
...
Our next goal is to show that if $\gammasm > 0$, then $\lim_{k\to\infty} \chik = 0$.    \sbnote{I think you should move on to the next section here since you are done with the initial criticality convergence.    Maybe also say ``To prove this will be considerably more.  We begin the development in the following sections''}
...
\subsubsection{Simplified Results}
\label{simplifed_bounded_projection}
\sbnote{What do you mean by the section title ``Simplified Results'' ?   What are they simplified from?}
...
If $\gammasm = 0$, then
\begin{align*}
\liminf_{k\to\infty} \left\|\proj_{\truefeasiblek}\left(\xk - \gradf\left(\xk\right)\right) - \xk \right\| = 0.
\end{align*}

If $\gammasm > 0$, then
\begin{align*}
\lim_{k\to\infty} \left\|\proj_{\truefeasiblek}\left(\xk - \gradf\left(\xk\right)\right) - \xk \right\| = 0.
\end{align*}
...
Here we discuss the numerical results of this algorithm when run on the Hott-Schittkowski problem set \cite{Schittkowski:1987:MTE:27135}.
...
\item At times, there are multiple optimum solutions, I currently only compare the result to the first contained in the Hott-Schittkowski problem set.
...
In practice, a maximum value of $\alpha$ and $\beta$ can be used for large $\dk$.
...
We gave each problem a starting ellipsoid: whenever possible we used a sphere centered around $\xinit$ of radius $1$.
Otherwise, we applied a heuristic that approximated a feasible direction and construct an ellipsoid near $\xinit$.

\subsubsection{Optimization Subroutines}
All optimization subroutines, such as the trust region subproblem were implemented in Pyomo.
In most cases, hot starts were provided from naive searches to improve the results the Pyomo solvers provided.
...
Ideally, this would use information returned from the inner optimization program to more intelligently explore ellipsoids.
...
In order to alleviate descent directions being removed from $\capcones$, we experimented with replacing the second order cones with quadratic buffers.
Namely, we replaced \cref{define_fik}
with 
\begin{align*}
...
\textrm{s.t.} & \beta (s-\wik)^T\left(I - u^{(i)}\left(u^{(i)}\right)^T\right) (s-\wik)\le \left(u^{(i)}\right)^T\left(s-\wik\right) & \forall i \in \activeconstraintsk.
\end{array}
\end{align*}
...
We evaluated enough points to construct quadratic models the constraints, because we constructed a quadratic model of the objective.
It is therefore possible to buffer the constraints with a quadratic term added to our models.
We include a comparison against this implementation.

Namely, we 
\begin{align*}
\begin{array}{ccc}
\min_{s \in \outertrk} & \mfk\left(\xk + s\right) & \\
\textrm{s.t.} & \mfk\left(\xk + s\right) + \beta \dk^{p_{\beta}}\left\|s - \xk\right\|^2 \le 0 & \forall i \in \activeconstraintsk.
\end{array}
\end{align*}

\subsection{Hott Schittkowski Problem Set}

To test our algorithm, we used the Hott-Schittkowski problem set \cite{Hock1980} and \cite{Schittkowski:1987:MTE:27135}.
...
The columns are 
\begin{itemize}
\item The Hott-Schittkowski problem number
\item The algorithm
...
\item Whether the minimum found was close to the minimum provided in the library \cref{is_optimal}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=300px]{images/nonlinear_performance_profile.png}
...
\begin{boxedcomment}
This should be {\em any} solution, rather than just {\em the}.
I only compare to the first solution, but sometimes there are multiple.
\end{boxedcomment}

\begin{boxedcomment}
Define what interesting means and give performance plot examples.
\end{boxedcomment}
...
However, we have noticed that in some problems (such as number, number, number) when the search path travels along constraint nearly orthogonal to the negative of the objective gradient.
This is because the buffering cones limit descent directions, and effectively place a cap on the largest possible trust region radius.
...
It claims that an well conditioned $\qk$ is found at any point after the trust region radius is sufficiently small, then all subsequent ellipsoids are conditioned.
