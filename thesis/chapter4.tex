...
The other evaluation that may be infeasible is the trial point.
...
Thus, we replace the classic trust region $\searchtrk = \feasiblek \cap \trk$:
\begin{align}
\label{original_tr_subproblem}
\begin{array}{ccccc}
\slin &=& \argmin_{s} & m_f(\xk + s) & \\
&& \textrm{s.t.} & m_{c_i}(\xk + s) \le 0 & \forall i \in \activeconstraintsk \\
&&&s \in \trk &
\end{array}
\end{align}
with $ \searchtrk = \capcones \cap \trk$:
\begin{align}
...
Notice that not searching all points within $\outertrk \cap \feasible$, we risk not providing sufficient reduction.
...
Namely, we introduce a constant 
\begin{align}
0 < \tolsr < \frac 1 2
\end{align}
and check 
...
\end{align}
Notice that this ensures the efficiency condition \cref{efficiency} is satisfied with a constant $\kappa_f \gets \tolsr \kappa_{f}$.
Also, this check can be performed without evaluating the current iterate.
\cref{sufficient_reduction_theorem} ensures this check can fail only a finite number of times.
The value of $\frac 1 2$ was chosen for convenience, and in practice any value in $(0,1)$ could work.
...
\begin{align}
0 < \tolsr < \frac 1 2 \label{define_tolsr}
\end{align}
...
%             Compute $\chik$ as in \cref{define_criticality_measure}.
        ...
        \item[\textbf{Step 3}] \textbf{(Solve the trust region subproblem)} \\
        	...
        	\cref{define_u}, \cref{define_thetamink}, \cref{define_bsk}, \cref{define_fik}, \cref{define_capcones}, \cref{define_rotation}, \cref{original_tr_subproblem} \cref{capcones_tr_subproblem}. \\
...
\subsubsection{Initial Criticallity Convergence}
...
\subsubsection{Criticallity Goes to Zero}
