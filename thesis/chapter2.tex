...
\subsection{Finding the Maximal $ \ellipsek $ Given $\mu^k$}
...
Because the ellipsoid we construct must be feasible with respect to both the trust region and the constraints, we simplify notation by creating a matrix $G$ and vector $g$ defined as follows:
\begin{align}
P_k = \{x \in \Rn | \quad Gx \le g, \|x - \xk\|_{\infty} \le \dk \} \label{polyhedron_k} \\
= \{x \in \Rn | \ak x \le \bk, \|{\ak}_i\| = 1 \} \nonumber.
\end{align}
